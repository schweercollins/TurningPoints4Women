...
dbs_hdl_df %>%
  ...
  annotate("text", x = 32.5, y = 15, label = "Percentile") +
  annotate("text", x = 43, y = 15, label = "Clinical")
...
pef_scale_df <- data.frame(
  ...
  scale = c("Average peak expiratory flow (3)",
...
pulse_scale_df <- data.frame(
  ...
  scale = c("Average resting pulse (2)",
...
total_scale_df <- data.frame(
  var_name = c("total_al"),
  scale = c("Total allostatic load score (13"),
...
Raw RHS variables were recoded per the [RHS-SF36 scoring guide](https://www.rand.org/health-care/surveys_tools/mos/36-item-short-form/scoring.html). All recoded variables end with `_p` and range from 0 to 100. Higher scores indicate a more favorable health state. Recoded variables were used to calculate the subscales. 

The missing data rule was not applied since all participants had less than 20% missing data. 

**<MARIA> INSERT REFERENCE**
...
Using the recoded RHS variables (`_p`), subscales were computed by averaging individual items per the [RHS-SF36 scoring guide](https://www.rand.org/health-care/surveys_tools/mos/36-item-short-form/scoring.html).

```{r rhs-comp-tbl, results = 'asis'}
#create table of subscale info
rhs_scale_df <- data.frame(
  var_name = c("rhs_health",
               "rhs_phys_limit",
               "rhs_emo_limit"),
  scale = c("General health (4)",
            "Role limitations due to physical health (4)",
            "Role limitations due to emotional problems (3)"),
  scale_construction = c("mean(`rhs9_p`, `rhs10_p`, `rhs11_p`, `rhs12_p`)",
                         "mean(`rhs2_p`, `rhs3_p`, `rhs4_p`, `rhs5_p`)",
                         "mean(`rhs6_p`, `rhs7_p`, `rhs8_p`)"),
  scale_range = c("0-100", "0-100", "0-100"))  

#create gt table
rhs_scale_table <- rhs_scale_df %>%
  gt() %>% 
  cols_align(columns = c("scale_range"), align = "center") %>% 
  cols_label(var_name = "Variable Name",
...
#apply function to calculate summary stats and create gt table
rhs_sum_tbl <- bio_df %>% 
  select(rhs1, rhs9:rhs_emo_limit) %>%
...
```{r rhs-avg-plot}
p_rhs_health <- bio_df %>%
  mutate(across(rhs_health, ~ifelse(. == -99 | . == -55 , NA, as.numeric(.)))) %>% 
  composite_hist(x = rhs_health) + 
  labs(title = "General Health") + 
  theme(plot.title = element_text(hjust = 0.5))

p_rhs_phys <- bio_df %>%
  mutate(across(rhs_phys_limit, ~ifelse(. == -99 | . == -55 , NA, as.numeric(.)))) %>% 
  composite_hist(x = rhs_phys_limit) + 
  labs(title = "Physical Limitations") + 
  theme(plot.title = element_text(hjust = 0.5))

p_rhs_emo <- bio_df %>%
  mutate(across(rhs_emo_limit, ~ifelse(. == -99 | . == -55 , NA, as.numeric(.)))) %>% 
  composite_hist(x = rhs_emo_limit) + 
  labs(title = "Emotional Limitations") + 
  theme(plot.title = element_text(hjust = 0.5))
  
#arrange
ggarrange(p_rhs_health, p_rhs_phys, p_rhs_emo) %>% 
  annotate_figure(top = text_grob( "Distribution for RHS Subscales"))

```

#### Clinical Categories

*<MARIA> INSERT CLINICAL DISTRIBUTION TABLE, IF APPLICABLE*

### COVID-19 Exposure and Vaccination (CEV) Description

The COVID-19 Exposure and Vaccination (CEV) is a self-report measure with 10-items assessing ... *<MARIA INSERT DESCRIPTION OF CEV>*

There were no subscales computed since the CEV is not a standardized measure with developed subscales but meant to assess COVID-19 exposure. *<MARIA CONFIRM/CORRECT>* Similarly, no distributions or clinical categories are presented as no subscales were calculated. 

**<MARIA> INSERT REFERENCE (or mention no reference if it was developed for this study**
...
The Chronic Health Conditions (CHC) survey is a self-report measure designed to assess various health conditions that could impact the biological data collection. *<MARIA CONFIRM/CORRECT>*
