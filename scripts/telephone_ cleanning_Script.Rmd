...
# TODO:Min, since d4 and d5 were reverse scored they should be included in the total, it looks like they are not included here but should be
dast_complete <-
  dast_calc %>%
  select(-missing_n,
         -miss_pct) %>%
  index_total_fun(
    id = c('id',
           'dast_screen',
           'd4', # Min possibly remove these here
           'd5')
  ) %>%
  rename(
...
## need to add back in all participants and missing data
```
...
### IBQ Missing Data 
There are missing data, hoever missing data rule not applied given this is a total sum of the number of systems endorsed 
```{r ibq_missing }
# correct missing value: Not Applicable = -55 = The subject was never asked a question for some reason (conditional question)
ibq <- ibq %>%
  # partial finished TC
