...
dyadc_complete <- dyadc_complete %>%
  select(id, dyadc_con, dyadc_aff_exp, dyadc_satis, dyadc_cohes, dyadc_total) # TODO: Maria, Please check I didn't miss anything 
...
flo_complete <-
  ...
  rename(flo_total = average_values) %>% # Min I changed to average_values as it should be an average not sum
  # replacing total score with -99 because due to 25% missing data this score isn't valid (follows missing data rule)
  mutate(flo_total = ifelse(id == "P847" & (flo_total == 78), -99, flo_total)) %>%
