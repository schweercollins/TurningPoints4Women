...
ibq_complete <- ibq_complete %>%
  select(id, ibq10_school:ibq10_other_text, ibq_total) %>% # dropped ibq10_77 no values
  merge(ibq, by="id") %>%
...
pcl <- pcl %>%
  mutate_at(vars(matches("pcl\\d+_77")), ~ifelse(is.na(.), 0, .)) %>%
  mutate_at(vars(matches("pcl\\d+_hapWit_ages_77")), ~ifelse(is.na(.), 0, .)) %>%
  mutate_at(vars(matches("pcl\\d+_hapWit_dev")), ~ifelse(is.na(.), 0, .)) 


# pcl_miss <- pcl %>%
#   mutate(across(-c(id, matches("_text$"), matches("^pcl\\d+relation"),matches("^pcl\\d+learned_relation"), matches("^pcl\\d+freq")), as.numeric)) %>%
...
# Missing data for only _happen and _ witness 
# 
