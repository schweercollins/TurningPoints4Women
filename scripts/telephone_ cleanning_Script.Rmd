...
# turn negative number to NA for calculation 
dast_calc <-
  dast %>% 
  mutate(across(-id, as.numeric)) %>%
  select(-dast_screen,
...
itq <- itq %>%
  ...
  #TODO: Maria, please double check that hr 173 did not do this measure
  mutate(across(-id, ~ifelse(id == "HR173", -99, .))) 
...
# We are not applying missing rule on this measure
```
