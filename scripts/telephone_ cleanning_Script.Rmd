...
ibq_miss <- ibq %>%
  ...
  pct_miss_fun(
    id = c("id", "ibq10_school", "ibq10_juvenile_justice", "ibq10_Legal_System", "ibq10_Police", "ibq10_Medical", "ibq10_Church", "ibq10_Foster_Care", "ibq10_Child_Welfare","ibq10_other","ibq10_other_text"),
    n_items = 10
  )
...
sex_miss <- sex %>%
  # mutate(across(-c(s2,s5,s7,matches("_text$")), as.numeric))%>%
  pct_miss_fun(
...
cint_ladder_remove <- cint_ladder_miss %>% filter(miss_pct >= 20) %>% pull(id)
...
cint_substance_complete <-
  ...
  mutate(
         across(
             ...
             ~case_when(
                 id %in% cint_substance_remove ~ NA_real_,
                 TRUE ~ .x
               )
           )
  )
