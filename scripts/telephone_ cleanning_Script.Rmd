...
```{r bsi_missing }
# correct missing value: Not Applicable = -55 = The subject was never asked a question for skip reason (conditional question)
bsi <- bsi %>% 
  mutate(
    bsi4 = case_when(
...
cesd_complete <-
  ...
  composite_total_avg_fun(
    id = c('id',
           ...
           'c8',
           'c12', 
           'c16', # c4,c8,c12, c16 were removed because we use reverse scored items instead 
           'c21', # C21 - c24 removed because they address suicidality 
           'c22',
           'c23',
           'c24'), 
    n_items = 20
...
dast <- dast %>% 
  # participants who were incarcerated at the time of survey completion were not adminstered this survey
...
## merging for original missing data codes
## changing prefix for measure

bcap_completeTest <- bcap_complete %>%
  ...
  # add in -99 for case in which subscales and scales not computed
  mutate_at(vars(everything()), ~ ifelse(is.na(.), -99, .))
...
prss_complete <- prss_calc %>%
  composite_total_avg_fun(
    id = 'id',
    max_value = 4, # Min, should this be 5 or 4?
    n_items = 5
  ) %>%
  distinct(
    across(
...
# set to 25% because this will effectively remove participant who is missing too much data for specific subscale
...
ibq <-
  ...
  mutate(ibq10 = rowSums(select(., ibq10_1:ibq10_9), na.rm = TRUE))%>% # ibq 10, how many specific systems they selected
# Min, we may consider ibq = 1 they selected systems; 0 none selected and then create a new variable that shows the total number of systems selected
...
ibq_complete <- ibq_complete %>%
  select(id, ibq10_school:ibq10_other_text) %>% # dropped ibq10_77 no values
  merge(ibq, by="id") %>%
...
# Maria, if merge code above works, we don't need this code in comment 
...
# Remove identifiable information columns 
sus_complete <- sus %>%
...
pcl <- pcl %>%
  ...
  rename_with(
   ...
   rename_with(
    .fn = ~ gsub("b1_\\d+", "_freq0-5", .),
    .cols = matches("pcl\\d+b1_\\d+_text")
  ) %>%
 rename_with(
    .fn = ~ gsub("b2_d+", "_freq6-11", .),
    .cols = matches("pcl\\d+b2_\\d+_text")
  ) %>%
  rename_with(
    .fn = ~ gsub("b3_d+", "_freq12-19", .),
    .cols = matches("pcl\\d+b3_\\d+_text")
  ) %>%   
  rename_with(
    .fn = ~ gsub("b4_d+", "_freqadult", .),
...
```{r}
pcl <- pcl %>%
    mutate_at(vars(pcl1_happen, pcl1_77), as.numeric) %>%
    mutate(pcl1_happen = ifelse(is.na(pcl1_happen) & is.na(pcl1_77), 0, ifelse(pcl1_77!= NA, -77, pcl1_happen)))

    mutate(pcl1_witness = ifelse((is.na(pcl1_witness) & pcl1_77 == -77), -77, ifelse(is.na(pcl1_witness), 0, pcl1_witness)))
    
  mutate_at(
    vars(matches("^pcl\\d+_happen$|^pcl\\d+_witness$|^pcl\\d+_learn$|^pcl\\d+_no$")),
    ~ ifelse(is.na(.) & get(paste0("pcl", gsub("[^0-9]", "", .), "_77")) == -77, -77, ifelse(is.na(.), 0, .))
  )
```
### PCL Missing 
```{r pcl_missing }
