...
# Min should we remove this? 
# mutate(across(all_of(c("c4", "c8", "c12", "c16")), ~ ifelse(. %in% c(-77, NA), ., 3 - .), .names = "{.col}_r"))
```

### CESD Missing Data
There are missing data in CESD and missing rule is not applied.
```{r CESD_missing }
# correct missing value: Not Applicable = -55 = The subject was never asked a question for some reason (conditional question)
...
ciq_complete <- 
  ...
  rename(
    ciq_psych_avg = mean_values ## average based on other publications
    ) %>%
...
### DAS Missing Data
There are missing data in DAS and missing rule applied. 
```{r dyadc_missing}
dyadc <- dyadc %>%
  # if answered no for screening question, the entire survey is skipped.
...
# TODO :Maria:JP also covert number larger than 4 to 4 in this code, but the SPSS Syntax cap at 7 which is what I used below. If this is correct, please delete this comment. 
    across(
      .cols = matches("\\d$"),   # TODO: Maria, JP used column end with a to create this receded column, but based on spss I think it should be column without a, this is what I used for this code. Please confirm this is correct. 
      .names = "{.col}_r"
    ),
    across(
      c(
...
# TODO: Maria, please double check right items were used for calculation 
ell_complete <-
  ell_calc %>%
  subscale_create(
    total_only = TRUE,
  ...
  mutate(
    across(
      ...
      .fns = ~case_when(
        ...
        # TODO: Maria, the code book guideline is: If nominal, number sequentially starting at 1 alphabetically, do you want me change the order 
        .x == "Cisgender female or woman" ~ "1",
        .x == "Female/Woman" ~"2",
        .x == "Cisgender male or man" ~ "3",
        .x == "Male/Man" ~ "4",
        .x == "Trans female/woman" ~ "5",
        .x == "Trans male/man" ~ "6",
        .x == "Non-binary"~ "7",
        .x == "Prefer to self-identify" ~ "8",
        ...
        .x == "1 - friend" ~ "1",
        .x == "2 - romantic partner" ~ "2",
        .x == "3 - sister or brother" ~ "3",
        .x == "4 - other relative" ~ "4",
        .x == "5 - counselor, teacher, doctor" ~ "5",
        .x == "6 - parent/parent figure" ~ "6",
        .x == "7 - other (please describe:)" ~ "7",
        ...
        # sex_std_happen ~ s24
        # TODO: Maria, please check if you want to code following option the way I did
        ...
        .x == "Don't know" ~ "9",
        .x == "10       Not using birth control because I am currently pregnant" ~ "10",
        # sex_oralsex_dentaldam_female_partner ~ s46
        .x == "Yes, had oral sex without a dental dam" ~ "1",
        .x == "No, oral sex and always used dental dam" ~ "0",
        .x == "Decline to Answer" ~ "-77",
        .x == "decline to answer" ~ "-77",
        .x == "Decline to answer" ~ "-77",
...
```{r sex_missing}
 sex <- sex %>% 
  ...
  # select(-c(s25_1:s25_77, s32_1:s32_77)) %>%
  ...
  # skip rule: if s13 is not Yes-1, s14:s17 is skipped  
   mutate_at(vars(s14:s17), ~case_when(s13 != 1 ~ "-55", TRUE ~ . )) %>%
    # skip rule: if s13 is not Yes-1, s14 is skipped  
   #mutate_at(vars(s14), ~case_when(s13 != 1 ~ "-55", TRUE ~ . )) %>%
...
# TODO: Maria, I can't figure out why NAs in s29 are missing 
  select(-c("s25_1","s25_2","s25_3","s25_4","s25_5","s25_6","s25_7","s25_8","s25_9","s25_77")) %>%
  select(-c("s32_1","s32_2","s32_3","s32_4","s32_5","s32_6","s32_7","s32_8","s32_9","s32_77"))
...
sex_miss %>% 
  gt::gt() %>% 
  gt::tab_header(
    title = 'sex Missing Data',
    subtitle = 'By Participant')

sex_remove <- sex_miss %>% filter(miss_pct >= 20) %>% pull(id)  
...
sex_complete <-
  sex %>%
  mutate(across(-c(id, s2,s5,s7, matches("_text$")), as.numeric)) %>%
  #mutate(across(-c(id,matches("_text$")), as.numeric)) %>%
  mutate(
    across(
      .cols = -c(id),
      .fns = ~case_when(
...
# Combine cint1 answer options to cint1 
...
cint_ladder_remove <- cint_ladder_missing %>% filter(miss_pct >= 20) %>% pull(id)
...
cint_ladder_complete <-
  ...
  mutate(
         across(
             -c(id),
             ~case_when(
                 id %in% cint_ladder_remove ~ NA_real_,
                 TRUE ~ .x
               )
           )
  )
...
cint_relation <- cint_relation %>%
  ...
  # if cint35 is none/1 or -77 cint36:cint37 will not be asked 
   ...
   mutate_at(vars(cint32_7_text), ~case_when(cint32 != 7 ~ "-55", TRUE ~ .))
...
cint_relation_remove <- cint_relation_miss %>% filter(miss_pct >= 20) %>% pull(id)
...
cint_relation_complete <-
  ...
  mutate(
         across(
             -c(id, matches("_7_text$")),
             ~case_when(
                 id %in% cint_relation_remove ~ NA_real_,
                 TRUE ~ .x
               )
           )
  )
...
cint_substance <- cint_substance %>%
  ...
  # cint40 they could choose not to answer 
  mutate(cint40 = ifelse(is.na(cint40), -77, cint40))%>%
  # cint44 they could choose not to answer 
  mutate(cint44 = ifelse(is.na(cint44), -77, cint44))%>%
    # cint53they could choose not to answer 
mutate(cint53 = ifelse(is.na(cint53), -77, cint53))%>%
      # cint83they could choose not to answer 
...
# TODO: Maria, I don't know why HR166 has cint84 missing 
...
cint_substance_remove <- cint_substance_miss %>% filter(miss_pct >= 20) %>% pull(id)


# Below is the code that treats -77, -55 values as NA
cint_substance_complete <-
  cint_substance %>%
mutate(across(-c(id, matches("_6_text$"), cint84), as.numeric)) %>% # cint84 is multiple choices
  mutate(
    across(
  ...
  mutate(
         across(
             -c(id, matches("_6_text$")),
             ~case_when(
                 id %in% cint_substance_remove ~ NA,
                 TRUE ~ .x
               )
           )
  )
...
```{r}
  # TODO: Maria, on Qualtrics, I saw there is an answer option N/A, but I didn't see column for NAs. I compired couple of them with Qualtric response, seems like all the empty ones are N/A so I coded all the empty ones to NA, Let me know if this should be code to something else. 
cint_covid <- tpw_raw %>%
...
cint_covid_miss%>% gt::gt() %>% 
  gt::tab_header(
    title = 'CINT- COVID Question Missing Data',
    subtitle = 'By Participant')

cint_covid_remove <- cint_covid_miss %>% filter(miss_pct >= 20) %>% pull(id)
...
cint_covid_complete <-
  ...
  mutate(
    across(
      -c(id),
      ~case_when(
        id %in% cint_covid_remove ~ NA_real_,
        TRUE ~ .x
      )
    )
  )
...
TODO: Min remove missing, no calculations is needed 
```{r socinf_gen_calc}
socinf_gen_complete <-
  socinf_gen %>%
  mutate(across(-c(id), as.numeric)) %>%
...
cts_pc_miss <- cts_pc %>%
  # Shaina: I was expected pct_miss_fun would count -2 but it didn't 
  mutate(across(-c(id), as.numeric))%>%
  pct_miss_fun(
    id = c("id", -matches("\\d$")),
    n_items = 24
...
# TODO: Maria, please pay extra attention to this section, since I use SPSS code as reference 
cts_pc_calc <-
  cts_pc %>%
    mutate(across(-c(id), as.numeric)) %>%
  mutate(
...
cts_pc_complete <-
  ...
  mutate(
    ...
    mutate(
      ...
      cts_pc_PSAP = rowSums(select(., 'a6_r', 'a8_r', 'a9_r', 'a10_r', 'a14_r'), na.rm = T), 
      # Count of Psychological Aggression Responses
      cts_pc_PSAC = rowSums(!is.na(select(., 'a6_r', 'a8_r', 'a9_r', 'a10_r', 'a14_r'))),
      # Raw Scoring for Psychological Aggression Past Year - Mean (Chronicity)
      cts_pc_PSAM = cts_pc_PSAP/cts_pc_PSAC,
      ...
      # Raw Scoring for Psychological Aggression Discipline - Lifetime (Ever Prevalence)
  ...
  mutate(
    ...
    cts_pc_PHAP = rowSums(select(., 'a3_r', 'a4_r', 'a7_r', 'a11_r', 'a13_r'), na.rm = T), 
    # Count of Physical Assault Responses
    cts_pc_PHAC = rowSums(!is.na(select(., 'a3_r', 'a4_r', 'a7_r', 'a11_r', 'a13_r'))),
    # Raw Scoring for Physical Assault Past Year - Mean (Chronicity)
    cts_pc_PHAM = cts_pc_PHAP/cts_pc_PHAC, 
    # Raw Scoring for Physical Assault - Past Year (Prevalence)
    cts_pc_PHAPP =  rowSums(select(., 'a3_r', 'a4_r', 'a7_r', 'a11_r', 'a13_r') > 0, na.rm = T),
    # Raw Scoring for Physical Assault - Lifetime (Ever Prevalence)
  ...
  # WKD Items: 16, 17, 18
  # Maria: WKD items need extra attention 
  mutate(
    ...
    cts_pc_WKDM = cts_pc_WKDP/cts_pc_WKDC,
  ...
  mutate(
    ...
    cts_pc_NEGP = rowSums(select(., 'a19_r', 'a20_r', 'a21_r', 'a22_r', 'a23_r'), na.rm = T), 
    # Count of Neglect Responses
    cts_pc_NEGC = rowSums(select(., 'a19_r', 'a20_r', 'a21_r', 'a22_r', 'a23_r'), na.rm = T), 
    # Raw Scoring for Neglect Past Year - Mean (Chronicity)
    cts_pc_NEGM = cts_pc_NEGP/cts_pc_NEGC,
...
pct_miss <- pct %>%
  mutate(across(-c(id, pct2e, pct3e), as.numeric)) %>%
  pct_miss_fun(
    id = c("id", "pct2e", "pct3e"),
    n_items = 13
  )
...
pct_remove <- pct_miss %>% filter(miss_pct >= 20) %>% pull(id)
```
...
pct_complete <-
  pct_calc %>%
  # TODO: Maria, please double check the calculation were 
  mutate(pct_preg_total = rowSums(select(., pct2a:pct2d), na.rm = T)) %>%
  mutate(pct_childbirth_total = rowSums(select(., pct3a:pct3d), na.rm = T)) %>%
  mutate(
    across(
      -c(id
      ),
...
## PCL 
### PCL Select Variabl and Recode 
Min's question: Should I rename the columns? what should I code NA to - they are not necessaryly missing; should I identify NA manually (eg. if pcl1_1, pcl1_2, pcl1_3, pcl1_4 are all NAs)
```{r plc}
pcl <- tpw_raw %>%
  select(id, starts_with("pcl")) %>%
  mutate(pcl1_dev = combine_columns(., c("pcl1a_1", "pcl1a_2", "pcl1a_3", "pcl1a_4"), sep = ";") )%>%
  mutate(pcl1_dev = str_extract(pcl1_dev, "^[^;]+")) %>%
  relocate(pcl1_dev, .after = "pcl1a_77") %>%
  mutate(pcl2_dev = combine_columns(., c("pcl2a_1", "pcl2a_2", "pcl2a_3", "pcl2a_4"), sep = ";") )%>%
  mutate(pcl2_dev = str_extract(pcl2_dev, "^[^;]+")) %>%
  relocate(pcl2_dev, .after = "pcl2a_77") %>%
  mutate(pcl3_dev = combine_columns(., c("pcl3a_1", "pcl3a_2", "pcl3a_3", "pcl3a_4"), sep = ";") )%>%
  mutate(pcl3_dev = str_extract(pcl3_dev, "^[^;]+")) %>%
  relocate(pcl3_dev, .after = "pcl3a_77") %>%
  mutate(pcl4_dev = combine_columns(., c("pcl4a_1", "pcl4a_2", "pcl4a_3", "pcl4a_4"), sep = ";") )%>%
  mutate(pcl4_dev = str_extract(pcl4_dev, "^[^;]+")) %>%
  relocate(pcl4_dev, .after = "pcl4a_77") %>%
  mutate(pcl5_dev = combine_columns(., c("pcl5a_1", "pcl5a_2", "pcl5a_3", "pcl5a_4"), sep = ";") )%>%
  mutate(pcl5_dev = str_extract(pcl5_dev, "^[^;]+")) %>%
  relocate(pcl5_dev, .after = "pcl5a_77") %>%
  mutate(pcl6_dev = combine_columns(., c("pcl6a_1", "pcl6a_2", "pcl6a_3", "pcl6a_4"), sep = ";") )%>%
  mutate(pcl6_dev = str_extract(pcl6_dev, "^[^;]+")) %>%
  relocate(pcl6_dev, .after = "pcl6a_77") %>%
  mutate(pcl7_dev = combine_columns(., c("pcl7a_1", "pcl7a_2", "pcl7a_3", "pcl7a_4"), sep = ";") )%>%
  mutate(pcl7_dev = str_extract(pcl7_dev, "^[^;]+")) %>%
  relocate(pcl7_dev, .after = "pcl7a_77") %>%
  mutate(pcl8_dev = combine_columns(., c("pcl8a_1", "pcl8a_2", "pcl8a_3", "pcl8a_4"), sep = ";") )%>%
  mutate(pcl8_dev = str_extract(pcl8_dev, "^[^;]+")) %>%
  relocate(pcl8_dev, .after = "pcl8a_77") %>%
  mutate(pcl9_dev = combine_columns(., c("pcl9a_1", "pcl9a_2", "pcl9a_3", "pcl9a_4"), sep = ";") )%>%
  mutate(pcl9_dev = str_extract(pcl9_dev, "^[^;]+")) %>%
  relocate(pcl9_dev, .after = "pcl9a_77") %>%
  mutate(pcl10_dev = combine_columns(., c("pcl10a_1", "pcl10a_2", "pcl10a_3", "pcl10a_4"), sep = ";") )%>%
  mutate(pcl10_dev = str_extract(pcl10_dev, "^[^;]+")) %>%
  relocate(pcl10_dev, .after = "pcl10a_77") %>%
  mutate(pcl11_dev = combine_columns(., c("pcl11a_1", "pcl11a_2", "pcl11a_3", "pcl11a_4"), sep = ";") )%>%
  mutate(pcl11_dev = str_extract(pcl11_dev, "^[^;]+")) %>%
  relocate(pcl11_dev, .after = "pcl11a_77") %>%
  mutate(pcl12_dev = combine_columns(., c("pcl12a_1", "pcl12a_2", "pcl12a_3", "pcl12a_4"), sep = ";") )%>%
  mutate(pcl12_dev = str_extract(pcl12_dev, "^[^;]+")) %>%
  relocate(pcl12_dev, .after = "pcl12a_77")
...
# Happened and Witnessed first developed period 
  ...
  # mutate(pcl1_dev = str_extract(pcl1_dev, "^[^;]+")) %>%
  # relocate(pcl1_dev, .after = "pcl1a_77") %>%
dev_column_names <- paste0("pcl", 1:12, "a_", 1:4)
pcl <- pcl %>%
  mutate(across(all_of(dev_column_names), ~str_extract(combine_columns(., across(starts_with(cur_column()), sep = ";")), "^[^;]+")) %>%
  relocate(paste0(cur_column(), "_dev"), .after = str_replace(cur_column(), "a_", "a_77")) )

pcl <- pcl %>%
  mutate(
    across(
      # had you experienced and how
...
# fund a more effection way to write these 
  # mutate(pcl1_dev = combine_columns(., c("pcl1a_1", "pcl1a_2", "pcl1a_3", "pcl1a_4"), sep = ";") )%>%
  # mutate(pcl1_dev = str_extract(pcl1_dev, "^[^;]+")) %>%
  # relocate(pcl1_dev, .after = "pcl1a_77") %>%
  # mutate(pcl2_dev = combine_columns(., c("pcl2a_1", "pcl2a_2", "pcl2a_3", "pcl2a_4"), sep = ";") )%>%
...
# TODO: Maria since we have developed period for combination of happened and witnessed, the following code is irrelevant, please confirm and removed. # Min this is the "first" developmental period it happened in, the code I created, I believe, if you look at it, it is scanning each PCL item and seeing when it first happened. Now that you have that information can you confirm it isn't irrelevant?
  #   mutate(
  ...
  #     levels = c("0-5", "6-11", "12-19", "adulthood")
  #   ),
  #   #TODO: Maria please double check I changed 16 to 18 
...
# TODO: Maria, I think below are duplicated code, if so, you can go ahead remove them. 
# pcl <- pcl %>%
#   mutate(pcl1_devperiod = case_when(
#      # TODO Maria, please double check I replace _16 to _2, and seems like this code section is doing the same with above
#     !is.na(pcl1a_1) & pcl1_1 == "Happened to me" ~ "0-5",
#     is.na(pcl1a_1) & !is.na(pcl1a_2) & pcl1_1 == "Happened to me" ~ "6-11",
#     is.na(pcl1a_1) & is.na(pcl1a_2) & !is.na(pcl1_3) & pcl1_1 == "Happened to me" ~ "12-19",
#     is.na(pcl1a_1) & is.na(pcl1a_2) & is.na(pcl1a_3) & !is.na(pcl1a_4) & pcl1_1 == "Happened to me" ~ "adulthood",
#     TRUE ~ NA_character_),
#     pcl2_devperiod = case_when(
#       !is.na(pcl2a_1) & pcl2_1 == "Happened to me" ~ "0-5",
#       is.na(pcl2a_1) & !is.na(pcl2a_2) & pcl2_1 == "Happened to me" ~ "6-11",
#       is.na(pcl2a_1) & is.na(pcl2a_2) & !is.na(pcl2_3) & pcl2_1 == "Happened to me" ~ "12-19",
#       is.na(pcl2a_1) & is.na(pcl2a_2) & is.na(pcl2a_3) & !is.na(pcl2a_4) & pcl2_1 == "Happened to me" ~ "adulthood",
#       TRUE ~ NA_character_),
#       pcl3_devperiod = case_when(
#         !is.na(pcl3a_1) & pcl3_1 == "Happened to me" ~ "0-5",
#         is.na(pcl3a_1) & !is.na(pcl3a_2) & pcl3_1 == "Happened to me" ~ "6-11",
#         is.na(pcl3a_1) & is.na(pcl3a_2) & !is.na(pcl3_3) & pcl3_1 == "Happened to me" ~ "12-19",
#         is.na(pcl3a_1) & is.na(pcl3a_2) & is.na(pcl3a_3) & !is.na(pcl3a_4) & pcl3_1 == "Happened to me" ~ "adulthood",
...
# Happen to me 
# pcl <- pcl %>%
#   mutate(pcl1_yes = if_else(pcl1_1 == "Happened to me", 1, 0, missing = 0),
#            # Maria: i replaced pcl16_yes with pcl2_yes
#          pcl2_yes = if_else(pcl2_1 == "Happened to me", 1, 0, missing = 0),
#          pcl3_yes = if_else(pcl3_1 == "Happened to me", 1, 0, missing = 0),
#          pcl4_yes = if_else(pcl4_1 == "Happened to me", 1, 0, missing = 0),
#          pcl5_yes = if_else(pcl5_1 == "Happened to me", 1, 0, missing = 0),
#          pcl6_yes = if_else(pcl6_1 == "Happened to me", 1, 0, missing = 0),
#          pcl7_yes = if_else(pcl7_1 == "Happened to me", 1, 0, missing = 0),
#          pcl8_yes = if_else(pcl8_1 == "Happened to me", 1, 0, missing = 0),
#          pcl9_yes = if_else(pcl9_1 == "Happened to me", 1, 0, missing = 0),
#          pcl10_yes = if_else(pcl10_1 == "Happened to me", 1, 0, missing = 0),
#          pcl11_yes = if_else(pcl11_1 == "Happened to me", 1, 0, missing = 0),
#          # Maria: i replaced pcl116_1 with pcl12_1
#          pcl12_yes = if_else(pcl12_1 == "Happened to me", 1, 0, missing = 0),
#          pcl13_yes = if_else(pcl13_1 == "Happened to me", 1, 0, missing = 0),
#          pcl14_yes = if_else(pcl14_1 == "Happened to me", 1, 0, missing = 0),
#          pcl15_yes = if_else(pcl15_1 == "Happened to me", 1, 0, missing = 0),
#          pcl16_yes = if_else(pcl16_1 == "Happened to me", 1, 0, missing = 0),
#          pcl17_yes = if_else(pcl17_1 == "Happened to me", 1, 0, missing = 0),
#          pcl16_yes = if_else(pcl16_1 == "Happened to me", 1, 0, missing = 0),
#          pcl19_yes = if_else(pcl19_1 == "Happened to me", 1, 0, missing = 0)
#          )
# okay here is a basic by time period
```
