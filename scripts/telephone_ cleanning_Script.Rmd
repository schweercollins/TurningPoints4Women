...
ell_complete <- ell_calc %>%
  ...
  mutate(ell_total = rowSums(select(., i1:i10), na.rm = T)) #%>%
#
...
### Maira I don't know what do you mean for comment above, but I think the mhc_complete is what you want for final achrive data 
...
#################################### Maria I think code in this section should be removed ###################################
# pct_miss <- pct %>%
#   #mutate(across(-c(id), as.numeric)) %>%
#   pct_miss_fun(
#     id = c("id"),
