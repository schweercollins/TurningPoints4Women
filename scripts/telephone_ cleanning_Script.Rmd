...
cts_pc_miss <- cts_pc %>%
  mutate(across(-c(id), as.numeric))%>%
  select(id, a1:a23) %>% # Maria, I think missing percentage should calculate using a1:a23
  pct_miss_fun(
    id = c("id"),
...
pcl_calc <- pcl %>%
  ...
  mutate(pcl_total_witness = rowSums(select(., ends_with("_witness")), na.rm = T))%>%
  # apply missing rule 
  mutate_at(vars(pcl_total_happen), ~ ifelse(id %in% c(pcl_happen_remove$id), -55, .))%>%
  mutate_at(vars(pcl_total_happen), ~ ifelse(id %in% c(pcl_witness_remove$id), -55, .))
