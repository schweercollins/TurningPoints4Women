...
new_births_table <- comb_long_preg %>%
  drop_na(
    preg_result,
    curr_preg
    ) %>%
...
comb_long_preg %>% 
  group_by(id, preg_result) %>% 
  summarize(preg_result_count = n()) %>% 
  ungroup() %>% 
  drop_na() %>% 
  gt::gt()
```
...
remove_df %>%
  ...
  pivot_longer(
    cols = c(matches('preg_result$')),
    names_to = 'preg_result',
    values_to = 'preg_result_value'
  ) %>% 
  drop_na(preg_result_value) %>%
  # filter(preg_result_value == 'birth') %>% # use this to only see the births
  distinct(
    id,
    preg_result,
    preg_result_value,
  ...
  relocate(preg_result, .after = id) %>% 
  relocate(preg_result_value, .after = preg_result) %>% 
  arrange(id)
...
```{r}
kiddo_name <- 
  remove_df %>% # there is no column with biological children that are already born
  select(
    id,
    matches('name$')
  ) %>% 
...
comb_long_lat <-
  ...
  select(
    ...
    matches('latino$'),
    matches('mexican$'),
    matches('latino_other$'),
    matches('latino_other_text$'),
    matches('puerto_rican$'),
    matches('cuban$'),
    matches('latino_dontknow$')
  ) %>% 
  mutate(
    across(
      .cols = c(matches('latino$')),
      .fns = ~case_when(
        .x == 1 ~ 'Yes',
        .x == 2 ~ 'No',
        .x == 3 ~ 'Don\'t know',
  ...
  pivot_longer(
    cols = -c(matches('latino$'),
              id,
              latino_yn),
    names_to = 'kiddo_latino',
    values_to = 'specific_latino'
  ) %>% 
  drop_na(
  latino_yn,
  specific_latino
  ...
  separate(
    are_latino,
    into = c('type', 'number', 'remove'),
    sep = '_'
  ) %>% 
  select(-remove) %>% 
  filter(number != 'latino') %>% 
  unite(
    'are_latino',
    c('type', 'number'),
    sep = '_'
  ) %>% 
  mutate(kiddo_latino2 = kiddo_latino) %>% 
  separate(
    kiddo_latino2,
    into = c('type', 'number', 'remove'),
    sep = '_'
  ...
  mutate(
    same_kid_question = case_when(
      are_latino == kiddo_latino2 ~ TRUE,
      are_latino != kiddo_latino2 ~ FALSE,
      TRUE ~ FALSE
    )
  ) %>% 
  filter(same_kid_question == TRUE) %>% 
  # distinct(
  #   id,
  #   .keep_all = TRUE
  # ) %>%
...
comb_long_custody <-
  ...
  select(
    ...
    # filter(id == 'HR119') %>% View()
  mutate(
    sm_sc1_living_with = case_when(
      sm_sc1_living_with == 'Yes' ~ 1,
      sm_sc1_living_with == 'No' ~ 2,
      sm_sc1_living_with == 'Decline' ~ 3,
      is.na(sm_sc1_living_with) ~ NA_real_
    ),
    across(
      .cols = matches('living_with$'),
      .fns = ~case_when(
        .x == 1 ~ 'Yes',
        .x == 2 ~ 'No',
        .x == 3 ~ 'Decline',
  ...
  pivot_longer(
    cols = matches('living_with$'),
    names_to = 'kid_living',
    values_to = 'kid_live_yesno'
  ) %>% 
  drop_na(custody, kid_live_yesno) %>% 
  arrange(id)
...
date_diff_df$month_diff <- date_diff_df$month_compare %/% months(1)
