...
tpw_raw <- read.csv(here("data", "tpw_measure.csv")) %>%
  select(-X) %>%
  filter(start_date != "Start Date")

source(here("codebook_fun.R"))
...
```
## Questions: 
-Do we want add prompt for each measure? 
- missing data rule should be in it's own section on top of code book? 
- Should all missing value including (-88 question was not shown) count as missing; if not what if the question was not shown was triggered by decline to answer previous question 

## Question for codebook: 
Should I add skip rule to codebook? 
Should I add a note for question that were not been asked due to TC incarcerated, I have note in this script, if we we share this note their ID will be linked to their incarcerated states, but no other identifiable information, is this a concern?  
...
cesd %>%
  ...
  # No missing data found in the CESD measure. 
```
...
```{r dast_missing }
# correct missing value: Not Applicable = -88 = The subject was never asked a question for some reason (conditional question)
# if d00 is No-1, rest column should be NA(-88)
...
dast <- dast %>% 
  ...
  mutate_at(vars(d00:d20,d4_r, d5_r), ~case_when(id %in% c("HR211", "HR181", "P819") & is.na(.) ~ -88, TRUE ~ .)) %>%
  # Skip rule: if d00 == 1 (TC is pregnant) the rest survey are skipped
  mutate_at(vars(d1a:d20,d4_r, d5_r), ~case_when(d00 == 1 ~ -88, TRUE ~ . )) %>%
