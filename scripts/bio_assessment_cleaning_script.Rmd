...
bio_td <- bio_df %>% 
  ...
  mutate(bp_sys_avg = rowMeans(select(., bp1a, bp2a), na.rm = TRUE),
         ...
         bp_sys_pal = ifelse(bp_sys_avg >= quantile(bp_sys_avg, 0.75), 1, 0),
         bp_dia_pal = ifelse(bp_dia_avg >= quantile(bp_dia_avg, 0.75), 1, 0),
         pef_avg    = round(rowMeans(select(., starts_with("pef")), na.rm = TRUE), 1),
         pef_cal    = ifelse(pef_avg <= 319, 1, 0),
         pef_pal    = ifelse(pef_avg >= quantile(pef_avg, 0.75, na.rm = TRUE), 1, 0),
         wth_ratio    = round(iliac_crest_measure / hip_measure, 3),
         wth_cal = ifelse(wth_ratio >= 0.85, 1, 0),
         wth_pal = ifelse(wth_ratio >= quantile(wth_ratio, 0.75), 1, 0),
         bmi_score  = weight_measure / ((height_measure / 100)^2),
         bmi_cal    = ifelse(bmi_score >= 30, 1, 0),
         bmi_pal    = ifelse(bmi_score >= quantile(bmi_score, 0.75), 1, 0),
         pulse_avg  = rowMeans(select(., pulse1, pulse2), na.rm = TRUE),
         pulse_tavg = ifelse(!is.na(pulse1) & !is.na(pulse2), 1, 0),
  ...
  mutate_at(vars(rhs9, rhs10, rhs11, rhs12),
            ~ case_when(. == "Definitely true" ~ "1",
                        ...
                        . == "Definitely false" ~ "5",
                        TRUE ~ .)) %>% 
  mutate_at(vars(hbc16a, hbc17a, hbc18a, hbc19a),
            ~ case_when(. == "a. never" ~ "1", 
                        . == "b. .5-1 hour/day" ~ "2",
                        . == "c. 1-5 hours/day" ~ "3",
                        . == "d. 5 or more hours/day" ~ "4",
                        TRUE ~ .)) %>% 
  mutate_at(vars(hbc21, hbc23),
            ~ case_when(. == "a. 0" ~ "1",
  ...
  mutate(rhs1 = case_when(rhs1 == "1 - Excellent" ~ "1",
         ...
         cev12 = case_when(cev12 == "Johnson & Johnson" ~ "1",
                           ...
                           TRUE ~ cev12),
         hbc13 = case_when(hbc13 == "1 cigarette" ~ "1",
                           hbc13 == "2-5 cigarettes" ~ "2",
                           hbc13 == "1 pack" ~ "3",
                           hbc13 == "2 packs" ~ "4",
                           TRUE ~ hbc13),
         hbc15 = case_when(hbc15 == "1 cigarette" ~ "1",
                           hbc15 == "2-5 cigarettes" ~ "2",
                           TRUE ~ hbc13),
         hbc28 = case_when(hbc28 == "Mild" ~ "1", 
                           hbc28 == "Moderate" ~ "2",
                           hbc28 == "Vigorous" ~ "3",
                           TRUE ~ hbc28),
         chc1a = ifelse(chc1a == "Maybe", "3", chc1a)) %>% 
  #specify missing data
  mutate_all(~ ifelse(. %in% c("N/A - Lives Alone", "N/A - Not currently employed"), "-88", .)) %>% 
  mutate_all(~ ifelse(. %in% c("Decline to Answer"), "-66", .)) %>% 
    select(id, starts_with("bp"), starts_with("bmi"), starts_with("wth"), pef1, pef2, pef3, pef4, pef5, pef6,
           pef_avg, pef_cal, pef_pal, starts_with("pulse"), starts_with("rhs"), starts_with("cev"),
           starts_with("hbc"), starts_with("chc"))





#ADD SKIP LOGIC NA'S (ESP. HBC VARS WHEN 0 SOME = NA; also need to check CHC - check all missing values to make sure NA is truly missing)
