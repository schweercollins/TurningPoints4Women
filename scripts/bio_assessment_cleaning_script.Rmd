...
bio_td <- bio_df %>% 
  ...
  mutate(bp_sys_avg = ifelse(bp1a != -55 & bp2a != -55, rowMeans(select(., bp1a, bp2a), na.rm = TRUE), -55),
         ...
         pef_pal    = ifelse(pef_avg != -55, ifelse(pef_avg <= quantile(pef_avg, 0.25, na.rm = TRUE), 1, 0), -55),
  ...
  mutate(missing_data_proportion = rowMeans(select(., ends_with("_cal")) %>% 
      ...
      total_al = ifelse(missing_data_proportion < 0.20, 
                        rowSums(select(., ends_with("_cal")) == 1, na.rm = TRUE), 
                        NA)) %>% 
  rename(bp1 = bp1a,
         bp2 = bp1b,
         bp3 = bp2a,
