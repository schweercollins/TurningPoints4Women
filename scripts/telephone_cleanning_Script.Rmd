...
# Set a variable to control the chunks for archive
# Set to FALSE if generate for NIJ archive 
eval_chunks <- TRUE
```
...
```{r cint_soci}
# once we create new raw file we need to pull in glptx", "sex", "ethnic", "age_14_ex into the cleaned file and add to codebook
## this file, Min "Sociodem_wv14_archive.sav"
# make note this comes from the historical data
cint_socio <- tpw_raw %>%
  select(id,cint_0:cint_income_support, cint_identity, cint_identity_8_text) %>%#, glptx, sex, ethnic, age_14_ex) %>%
  mutate(
    across(
      .cols = c(cint2:cint5),
      .fns = ~case_when(
...
## we should P801 to see if there are values
