...
sex <- sex %>%
  mutate_at(vars(s10), ~ ifelse(s10_4_text %in% c("Non-discriminatory, \"I am not really sure, maybe pan sexual\"", "Pansexual"), 4, .)) %>%
  mutate_at(vars(s10), ~ ifelse(s10_4_text %in% c("Lesbian but open-minded."), 2, .)) %>% # 2 for lesbian/gay
  mutate_at(vars(s10), ~ ifelse(s10_4_text %in% c("Currently, male-leaning bisexual."), 3, .)) %>% # 3 for bi-sexual
