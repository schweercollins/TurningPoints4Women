...
tpw_parent_data <- read.csv(here("do_not_push", "ParentDataforArchive.csv")) %>%
  rename(id = FAM.ID)%>%
  select(-p00a, - X)
...
arrest_clean <- read.csv(here("do_not_push", "tpw_arrest_archive.csv")) %>%
...
arril <- arril %>%
  ...
  mutate(AgeLastArrestSR = ifelse(date_last_arrest_SR < 0, date_last_arrest_SR, AgeLastArrestSR)) %>%
...
cts_complete <- cts_complete %>%
  ...
  # We are not include midpoint variable becauey they were used to compute variable, but meaningless for analysis
...
itq <- tpw_raw %>% 
  select(
    ...
    across(
     ...
     #   is.na(.x) ~ -99, ## need to figure out how to get this right
        TRUE ~ .x
      )
    )
  ) 
...
itq_complete <- itq_complete %>%
  select(id, itq_re_total, itq_av_total, itq_th_total, itq_fi_total, itq_ptsd_total, itq_pstd_dx) %>%
  merge(itq, by="id", all.y = TRUE) %>%
  relocate(itq_re_total, itq_av_total, itq_th_total, itq_fi_total, itq_ptsd_total, itq_pstd_dx, .after = "itq9")
```
...
## we should P810 to see if there are values
# No data in 6a, 6b,6c for P810

## then remove 6a-6c from archive data and codebook
## removed because recoded into other values
...
tpw_archive_calc <- tpw_archive_id %>%
  left_join(tpw_parent_data, by = "id") %>%
  left_join(cls_data, by = "id") %>%
  left_join(arril, by = "id") %>%
  left_join(arrest_clean, by = "id") %>%
