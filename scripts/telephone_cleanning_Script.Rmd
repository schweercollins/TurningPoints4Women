...
# import parent data for archive 
parent_complete <- read.csv(here("do_not_push", "ParentDataforArchive.csv")) %>%
  rename(id = FAM.ID)%>%
  select(-p00a, - X) %>%
  slice(-1)


# import CLS data for archive 
cls_data <- read.csv(here("do_not_push", "tpw_cls_archive.csv")) %>%
  select(-X, -total_days_incarcerated, -total_days_homeless)

# import functions 
source(here("functions", "codebook_fun.R"))


# import historical variables
Sociodem_wv14_archive <- read_sav(here("do_not_push", "Sociodem_wv14_archive.sav")) %>%
  rename(id = FAMILY)

# import ARRIL data 
arril_clean <- read.csv(here("do_not_push", "tps_arril_clean.csv")) %>%
  #filter(id != "TC ID:") %>%
   # set negative values to NA 
  mutate(date_last_arrest_SR_NA = ifelse(date_last_arrest_SR < 0, NA, date_last_arrest_SR)) %>%
  mutate(date_last_arrest_SR_NA = as.Date(date_last_arrest_SR_NA, format = "%m/%d/%y"))

# import official arrest data
arrest_complete <- read.csv(here("do_not_push", "tpw_arrest_archive.csv")) %>%
  select(id, LastOffAge, LastOffAgeWith1, F18OffAge, F18OffAgeWith1, FF18OffAge, Num14NewOff,	Num14NewOffWith1, Num14NewFOff, Wave14offseverity, Wave14offseverityWith1, NumIncarWave14, TPDesist_with1andCLSYes) %>%
  rename(Wave14Desist = TPDesist_with1andCLSYes) %>%
  mutate_at(vars(starts_with("F18")), ~ifelse(is.na(.), -55, .)) %>%
  mutate_at(vars(starts_with("FF18")), ~ifelse(is.na(.), -55, .)) %>%
...
## ARRIL 
```{r arril}
# Merge arril with Sociodem_wv14_archive to get dob 
arril <- Sociodem_wv14_archive %>%
  select(id, dob) %>%
  merge(arril_clean, by = "id", all.y = TRUE) %>%
 # calculate AgeLastArrestSR for age of last arrest self-reported
 mutate(AgeLastArrestSR = as.numeric(difftime(date_last_arrest_SR_NA, dob, units = "days")) /365.25, date_last_arrest_SR) %>%
  select(id, ar00, ar00a,AgeLastArrestSR,date_last_arrest_SR) %>%
    mutate(
      across(
        .cols = c(ar00),
        .fns = ~case_when(
...
```{r arril-missing}
arril_complete <- arril %>%
  filter(id != "" & id != "TC ID:") %>%
  mutate(ar00 = ifelse(is.na(ar00), "-99", ar00))%>%
  mutate(ar00a = ifelse(ar00 != "1", "-55", ar00a))%>%
 mutate(AgeLastArrestSR = ifelse(date_last_arrest_SR < 0, date_last_arrest_SR, round(AgeLastArrestSR, 2))) %>%
  mutate(AgeLastArrestSR= ifelse(AgeLastArrestSR == "", -55, AgeLastArrestSR)) %>%
  select(-date_last_arrest_SR) %>%
  rename(YesArrestSR = ar00) %>%
  rename(NumArrestSR = ar00a)
```
...
bsi <- bsi %>% 
  mutate(
    bsi4 = case_when(
      ...
      id %in% c("HR211", "HR181", "P819", "HR218") & is.na(bsi4) ~ "-55",
      TRUE ~ bsi4
    )
  )
...
cesd <- cesd %>%
  mutate(c22 = ifelse((c21 == 0 |c21 ==  "-77"), "-55", c22),
         ...
         c21 = case_when( id %in% c("HR211", "HR181", "P819", "HR218") & is.na(c21) ~ "-55",TRUE ~ c21),
         c22 = case_when( id %in% c("HR211", "HR181", "P819", "HR218") & is.na(c22) ~ "-55",TRUE ~ c22),
         c23 = case_when( id %in% c("HR211", "HR181", "P819", "HR218") & is.na(c23) ~ "-55",TRUE ~ c23),
         c24 = case_when( id %in% c("HR211", "HR181", "P819", "HR218") & is.na(c24) ~ "-55",TRUE ~ c24)
      )
...
### DAST Missing Data 
There are missing data in DAST and the missing rule was applied. All but one instance of missing data is due to the skip logic.
```{r dast_missing }
...
dast <- dast %>% 
  ...
  mutate_at(vars(dast_screen:d20,d4_r, d5_r), ~case_when(id %in% c("HR211", "HR181", "P819", "HR218") & is.na(.) ~ "-55", TRUE ~ .)) %>%
...
dast_calc <-
  ...
  mutate(across(-id, as.numeric)) %>%
  select(-dast_screen,
         -d4, # dropping because we are using reverse scored d4 and d5
         -d5) %>%
  pct_miss_fun(
    id = 'id',
    n_items = 22
...
cts_complete <- cts %>%
  ...
  mutate_at(vars(cts_screen, ct1:ct20), ~ ifelse(id %in% c("HR211", "HR181", "P819", "HR218"), "-55", .))
...
#   # We are not include midpoint variable because they were used to compute variable, but meaningless for analysis
#   select(id, cts_psy_agg_ind_sev:cts_sex_mutual) %>%
#   merge(cts, by="id") %>%
#   # reorder dataset 
#   relocate(cts_psy_agg_ind_sev:cts_sex_mutual, .after = "ct20") %>%
...
## can you please let me know how many valid responses we have to this measure meaning dyadc_screener = 1
## can you also let me know how many total scores we have
## make sure to include the new P810 if possible
## if incarcerated had data and we removed them 
...
dyadc <- dyadc %>%
  ...
  mutate_at(vars(dyadc_screener, e1:e19_r), ~ ifelse(id %in% c("HR211", "HR181", "P819", "HR218"), "-55", .)) 
...
# Missing percentage for all 32 variables 
dyadc_miss <- dyadc %>%
    mutate(across(-id, as.numeric)) %>%
  pct_miss_fun(
...
dyadc_remove <- dyadc_miss %>% filter(miss_pct > 20) %>% pull(id)

# Missing percentage for Dyadic Consensus - 1, 2, 3, 5, 7, 8, 9, 10, 11, 12, 13, 14, 15 (65 points possible)
dyadc_dc_miss <- dyadc %>%
  mutate(across(-id, as.numeric)) %>%
  select(id, das1, das2, das3, das5,
...
dyadc_dc_miss %>%
  gt::gt() %>% 
  gt::tab_header(
    title = 'DYADC Dyadic Consensus Missing Data',
    subtitle = 'By Participant'
  )
dyadc_dc_remove <- dyadc_dc_miss %>% filter(miss_pct > 20) %>% pull(id)

# Missing percentage for Affectional Expression - 4, 6, 29, 30 (12 points possible)
dyadc_ae_miss <- dyadc %>%
  mutate(across(-id, as.numeric)) %>%
  select(id, das4, das6, das29, das30) %>%
  pct_miss_fun(
    id = c('id'),
    n_items = 4
  ) 
...
dyadc_ae_miss %>%
  gt::gt() %>% 
  gt::tab_header(
    title = 'DYADC Affectional Expression Missing Data',
    subtitle = 'By Participant'
  )
dyadc_ae_remove <- dyadc_ae_miss %>% filter(miss_pct > 33) %>% pull(id)

# Missing percentage for Dyadic Satisfaction - 16, 17, 18, 19, 20, 21, 22, 23, 31, 32 (50 points possible)
dyadc_ds_miss <- dyadc %>%
  mutate(across(-id, as.numeric)) %>%
  select(id, das16, das17, das18, das19, das20,
                 das21, das22, das23, das31, das32) %>%
  pct_miss_fun(
    id = c('id'),
    n_items = 10
  ) 
...
dyadc_ds_miss %>%
  gt::gt() %>% 
  gt::tab_header(
    title = 'DYADC Dyadic Satisfaction Missing Data',
    subtitle = 'By Participant'
  )
dyadc_ds_remove <- dyadc_ds_miss %>% filter(miss_pct > 20) %>% pull(id)

# Missing percentage for Dyadic Cohesion - 24, 25, 26, 27, 28 (24 points possible) 
dyadc_dc_miss <- dyadc %>%
  mutate(across(-id, as.numeric)) %>%
  select(id, das24, das25, das26, das27, das28) %>%
  pct_miss_fun(
    id = c('id'),
    n_items = 5
  ) 
...
dyadc_dc_miss %>%
  gt::gt() %>% 
  gt::tab_header(
    title = 'DYADC Dyadic Cohesion Missing Data',
    subtitle = 'By Participant'
  )
dyadc_dc_remove <- dyadc_dc_miss %>% filter(miss_pct > 30) %>% pull(id)
```

### DAS Calculations & Subscale
```{r dyadc_calculations}
# change all negative values to NA for calculation 
dyadc_calc <-
  dyadc %>%
  mutate(across(-id, as.numeric)) %>%
  mutate(
...
# Calculate total score for das: ## Total = all 32 items
dyadc_complete <-
  dyadc_calc %>%
  composite_total_avg_fun(
    id = c('id', 'dyadc_screener','das18', 'das19'), # we are using reverse score for das 18 and das19
    n_items = 32
  ) %>%
  distinct(
    across(
  ...
  # apply missing rule for dyadc_total
  mutate(
    across(
      .cols = dyadc_total,
      .fns = ~ case_when(
        id %in% dyadc_remove ~ -99,
        TRUE ~ .x
      )
    )
  )
...
dyadc_complete <-
  ...
  subscale_create(
    ...
    scale3 = c('das16', 'das17', 'das18_r', 'das19_r', 'das20',
               'das21', 'das22', 'das23', 'das31', 'das32'),
    scale3_nitems = 10,
    ## Dyadic Cohesion - 24, 25, 26, 27, 28 (24 points possible) 
    scale4 = c('das24', 'das25', 'das26', 'das27', 'das28'),
    scale4_nitems = 5
  ) %>%
  rename(
    dyadc_con = total1,
...
# DAS finalized data: add appropriate missing values back in
  # if answered no for screening question, the entire survey is skipped.
  # Incarcerated TC weren't administered for this survey
dyadc_complete <- dyadc %>%
  select(id, c(dyadc_screener, starts_with("das"))) %>%
  #rename_with(~sub("^e", "das", .), starts_with("e"))%>%
  rename(das_screener = dyadc_screener) %>%
  ...
  mutate(
    across(
      ...
      ~case_when(
        ...
        .x == "2-3 times a day" ~ "6",   
        .x == "Decline to Answer" ~ "-77",
        is.na(.x) ~ "-99", ## NA in this case is true missing
        TRUE ~ .x
      )
    ),
    ell23 = case_when(
...
ell <- ell %>%
  ...
  mutate(across(-id, ~ifelse(id == "HR173", -99, .))) %>%
   # Incarcerated TC weren't administered for this survey
  mutate_at(vars(ell1:ell39a), ~ ifelse(id %in% c("HR211", "HR181", "P819", "HR218"), -55, .)) %>%
...
ell_total_minor_offenses_miss <- ell %>%
    mutate(across(-id, as.numeric)) %>%
...
ell_total_minor_offenses_miss %>%
  gt::gt() %>%
  gt::tab_header(title = 'Missing Data',
                 subtitle = 'By Each Participant')

ell_total_minor_offenses_remove <- ell_total_minor_offenses_miss %>% filter(miss_pct >= 30) %>% pull(id)
...
ell_complete <- ell_calc %>%
  ...
  # total index of minor offenses (29 items but not 23; 1, 2, 3, 6, 7, 8, 9, 11, 12, 13, 14, 15, 17, 18, 19, 20, 21, 22, 24, 26, 27, 28, 30, 31, 33, 34, 35, 36, 37) 
  mutate(ell_total_minor_offenses = rowSums(select(., c(ell1, ell2, ell3, ell6, ell7, ell8, ell9, ell11, ell12, ell13, ell14, ell15, ell17, ell18, ell19, ell20, ell21, ell22, ell24, ell26, ell27, ell28, ell30, ell31, ell33, ell34, ell35, ell36, ell37)), na.rm = T)) %>%
  # apply missing rule to ell_total_index_minor_offenses
  mutate_at(vars(ell_total_minor_offenses), ~case_when(id %in% ell_total_minor_offenses_remove ~ -99, TRUE ~ .x )) 

  
ell_complete <- ell_complete%>%
  select(id, ell_total, ell_total_index_offenses, ell_total_minor_offenses) %>%
  merge (ell, by = "id", all.y = TRUE) %>%
  relocate(id, .before = "ell1") %>% 
  relocate(ell_total, ell_total_index_offenses,ell_total_minor_offenses, .after = "ell39a")
...
mhc_pa_remove <- mhc_pa_miss %>% filter(miss_pct >= 33) %>% pull(id) # No one needs to be removed 
...
mhc_pwb_remove <- mhc_pwb_miss %>% filter(miss_pct >= 33) %>% pull(id)  # No one needs to be removed 
...
```{r}
flo <- flo %>%
  # These variables were missing for this TC but not declined
...
flo_complete <- flo_calc %>%
  ...
  # "secure flourish": average of all items including i11 and i12
  mutate(secure_flourish = rowSums(select(., i1:i12), na.rm = T)) %>%
...
itq <- tpw_raw %>% 
  ...
  mutate(
    across(
      .cols = c(itq1:itq9),
      .fns = ~ {
        .x <- na_if(.x, "NA") # Replace "NA" strings with actual NA values
        .x <- case_when(
          .x == "Not at all" ~ "0",
          .x == "A little bit" ~ "1",
          .x == "Moderately" ~ "2",
          ...
          is.na(.x) ~ "-99",  # Replace NA values with -99
          TRUE ~ .x
        )
        return(.x)
      }
...
# This code can be used to compute diagnoses based on participant endorsing at least one symptom in each cluster per ITQ measure guidelines

## '1' (versus '2) selected in the current study because answer scale visual given to participants in the study was 1 = 1-2 times per week symptoms severity, 2 = 3-4 times per week, 3 = daily, 4 = multiple times daily; consistent with other trauma measures it would be an underestimate to not include 1; however, measure is also potentially flawed because there is only 0 or 1-2 times per week. Researchers could use more stringent criteria of 2 for symptom manifestation.
itq_calc <- itq_calc %>%
  mutate(
    itq_re_dx = case_when(
...
itq_complete <- itq_complete %>%
  mutate(
    itq_re_total = if_else(id == "HR173", -99, itq_re_total),
    itq_av_total = if_else(id == "HR173", -99, itq_av_total),
    itq_th_total = if_else(id == "HR173", -99, itq_th_total),
    itq_fi_total = if_else(id == "HR173", -99, itq_fi_total),
    itq_ptsd_total = if_else(id == "HR173", -99, itq_ptsd_total),
    itq_ptsd_dx = if_else(id == "HR173", "-99", itq_ptsd_dx)) %>%
  select(id, itq_re_total, itq_av_total, itq_th_total, itq_fi_total, itq_ptsd_total, itq_ptsd_dx) %>%
  merge(itq, by="id", all.y = TRUE) %>%
  relocate(itq_re_total, itq_av_total, itq_th_total, itq_fi_total, itq_ptsd_total, itq_ptsd_dx, .after = "itq9")
```
...
ibq <- ibq %>%
  # partial finished TC
  mutate(across(-c(id), ~ifelse(id == "HR173", "-99", .))) 
...
ibq_complete <- ibq_complete %>%
  mutate(ibq10_legal_system = if_else(id == "HR107" | id == "HR211", "1", as.character(ibq10_legal_system)),
         ibq10_other_text = if_else(is.na(ibq10_other) | ibq10_other == 0, "", ibq10_other_text),
         ibq10_other_text = if_else(id == "HR172", "County", ibq10_other_text),
         ibq10_other_text = if_else(id == "HR181", "[   ]", ibq10_other_text),
         ibq10_other_text = if_else(id == "HR214", "homeless shelter", ibq10_other_text),
         ibq10_other_text = if_else(id == "P805", "community organization", ibq10_other_text),
         ibq10_other_text = if_else(id == "P813", "community organization", ibq10_other_text),
         ibq10_other_text = if_else(id == "P839", "women's advocacy center", ibq10_other_text),
         ibq10_other_text = if_else(id == "P866", "prison system", ibq10_other_text),
         ibq10_other_text = if_else(id == "P885", "prison system", ibq10_other_text),
         ibq10_other_text = if_else(id == "P890", "state", ibq10_other_text),
         ibq10_other_text = if_else(id == "P895", "[   ]", ibq10_other_text),
         ibq10_other_text = if_else(is.na(ibq10_other_text) | ibq10_other_text == "", "-55", ibq10_other_text),
         across(ibq10_school:ibq10_other_text, ~if_else(is.na(.x), "-55", as.character(.x)))
  )
  ...
  mutate(
    across(
      ...
      .fns = ~case_when(
        ...
        .x == "4 - Other: Please describe" ~"4", # this is recode to Pansexual in later code 
...
# recode the s10_4_text to s10 
sex <- sex %>%
  mutate_at(vars(s10), ~ ifelse(s10_4_text %in% c("Pansexual"), 4, .)) %>%
  mutate_at(vars(s10), ~ ifelse(s10_4_text %in% c("Lesbian but open-minded."), 2, .)) %>% # 2 for lesbian/gay
  mutate_at(vars(s10), ~ ifelse(s10_4_text %in% c("Currently, male-leaning bisexual.", "Non-discriminatory, \"I am not really sure"), 3, .)) %>% # 3 for bi-sexual
  # add data from cint-relation cintq3 for incarcerated TC 
  mutate(s1 = ifelse(id == "HR181", 1, s1)) %>%
  mutate(s6 = ifelse(id == "HR181", 1, s6))
...
sex <- sex %>% 
  # for incarcerated TC, this survey wasn't shown 
   mutate_at(vars(s_pregnancy_screen:s48), ~case_when(id %in% c("HR211", "HR181", "P819", "HR218") & is.na(.)  ~ "-55", TRUE ~ .)) %>%
  ...
  mutate_at(vars(s12), ~case_when(id == "P813" ~ "-99", TRUE ~ . )) %>%
...
# we change all the date to first day of the month to keep confidentiality 
convert_to_date <- function(date_str) {
  if (grepl("^-?\\d{1,2}/\\d{4}$", date_str)) {  # Check if mm/yyyy format
    parts <- unlist(strsplit(date_str, "/"))
    return(paste("01", sprintf("%02d", as.numeric(parts[1])), parts[2], sep = "/"))
  } else if (grepl("^-?\\d{1,2}/\\d{1,2}/\\d{4}$", date_str)) {  # Check if mm/dd/yyyy format
    parts <- unlist(strsplit(date_str, "/"))
    return(paste("01", parts[2], parts[3], sep = "/"))
  } else {
    return(date_str)  # Return unchanged if not in expected formats
  }
}

# Apply the function to the 's2' and 's5' and 's7' columns

 sex$s2 <- sapply(sex$s2, convert_to_date)
 sex$s5 <- sapply(sex$s5, convert_to_date)
 sex$s7 <- sapply(sex$s7, convert_to_date)

 sex <- sex %>%
   # TC has two date entered we are only keep the first one to better align with the question
 mutate_at(vars(s5), ~ ifelse(id == "P898",  "01/04/2012", .)) 
```
...
sex_complete <- sex %>%
  ...
  select(-s32) #dropping because we recoded; intentionally still references in codebook though
```
...
# Warning just indicates that when a participant didn't have two choices, they were given a NA for choice 2
...
# historical data for demographic: "glptx", "sex", "ethnic", "age_14_ex"
# glptx, sex, ethnic, age_14_ex
cint_socio <- Sociodem_wv14_archive %>%
  select(id, GLPTX, sex, ETHNIC, age_14_ex) %>%
  rename(glptx = GLPTX, ethnic = ETHNIC) %>%
  merge(cint_socio, by = "id")
```
...
cint_socio_complete <- cint_socio 
```
...
cint_relation <- cint_relation %>%
  ...
  # if cint35 is none/0 or -77 cint36:cint37 was be asked 
   ...
   mutate_at(vars(cint30_7_text), ~case_when(cint30 != 6 ~ "-55", TRUE ~ .)) %>% 
   mutate_at(vars(cint31_7_text), ~case_when(cint31 != 6 ~ "-55", TRUE ~ .)) %>% 
   mutate_at(vars(cint32_7_text), ~case_when(cint32 != 6 ~ "-55", TRUE ~ .)) %>%
   mutate_at(vars(cint28,cint29), ~case_when(id == "P813" ~ "-99", TRUE ~.))
...
cint_substance <- cint_substance %>%
  ...
  mutate_at(vars(cint39:cint84), ~ ifelse(id %in% c("HR211", "HR181", "P819", "HR218"), -55, .)) %>%
...
socinf_sub <- socinf_sub%>%
  ...
  mutate_at(vars(socinf_preg_screener:socd4k), ~case_when(id %in% c("HR211", "HR181", "P819", "HR218") & is.na(.) ~ "-55", TRUE ~ .)) # they will therefore not have screener questions asked
...
socinf_gen <- socinf_gen %>%
  ...
  # Incarcerated participants weren't administered for this survey
  mutate_at(vars(sg1:sg13d, socinf_general_screener), ~case_when(id %in% c("HR211", "HR181", "P819", "HR218") ~ "-55", TRUE ~ .)) %>%
...
cts_pc_complete <- cts_pc_complete %>%
 ...
 # mutate(across(c(a1:a23, a1_r:a23_r), ~ifelse(cts_parent_screen == 0, -55, ifelse(is.na(.), -77, as.numeric(.))))) %>%
  ...
  relocate(cts_parent_screen:cts_pc23_r, .after = "id")
...
pct <- pct %>%  
  ...
  mutate_at(vars(pct2a:pct3e), ~ case_when(pct1 == "-77" ~ "-77", TRUE ~ .)) %>% # applies to one case
...
sus <- sus %>% 
  ...
  mutate(across(-id, ~ifelse(id == "HR173", -99, .))) %>%
  mutate(across(sus11a:sus26c, ~ifelse(id == "P810", -99, .)))%>%
  ...
  mutate(sus11a = case_when(is.na(sus11a) ~ "-77", TRUE ~ sus11a)) %>%
...
# P856 (add count of 3 to sus4)
# P887 (add count of 1 to sus1)
# P849 (add count of 3 to sus1)
# P882 (add count of 2 to sus1)
# P867 (Add count of 3 to sus1)
# P902 (Add count of 1 to sus1)
# P845 (add count of 3 to sus2)
# HR204 (Add count of 3 to sus2)
# HR131 (add count of 3 to sus4)
# HR 175 (add count of 3 to sus4)
#HR 171 (add count of 1 to sus3)
# P805 add count of 3 to sus7
#P829 add count of 1 to sus7
#P890 add count of 1 to sus1
sus <- sus %>%
  # removed and recede 6a,6b, and 6c because they include confidential information
  select(-sus6a, - sus6b, - sus6c) %>%
  mutate(across(c(sus1, sus2, sus3, sus4, sus7), as.numeric))%>%
  # P865 (add count of 2 to sus1)
  mutate(sus1 = ifelse(id == "P865", sus1 + 2, sus1)) %>%
  # P856 (add count of 3 to sus4)
  mutate(sus4 = ifelse(id == "P856", sus4 + 3, sus4)) %>%
  # P887 (add count of 1 to sus1)
  mutate(sus1 = ifelse(id == "P887", sus1 + 1, sus1)) %>%
  # P849 (add count of 3 to sus1)
  mutate(sus1 = ifelse(id == "P849", sus1 + 3, sus1)) %>%
  # P882 (add count of 2 to sus1)
  mutate(sus1 = ifelse(id == "P882", sus1 + 2, sus1)) %>%
  # P867 (Add count of 3 to sus1)
  mutate(sus1 = ifelse(id == "P867", sus1 + 3, sus1)) %>%
  # P902 (Add count of 1 to sus1)
  mutate(sus1 = ifelse(id == "P902", sus1 + 1, sus1)) %>%
  # P845 (add count of 3 to sus2)
  mutate(sus2 = ifelse(id == "P845", sus2 + 3, sus2)) %>%
  # HR204 (Add count of 3 to sus2)
  mutate(sus2 = ifelse(id == "HR204", sus2 + 3, sus2)) %>%
  # HR131 (add count of 3 to sus4)
  mutate(sus4 = ifelse(id == "HR131", sus4 + 3, sus4)) %>%
  # HR175 (add count of 3 to sus4)
  mutate(sus4 = ifelse(id == "HR175", sus4 + 3, sus4)) %>%
  # HR171 (add count of 1 to sus3)
  mutate(sus3 = ifelse(id == "HR171", sus3 + 1, sus3)) %>%
  # P805 add count of 3 to sus7
  mutate(sus7 = ifelse(id == "P805", sus7 + 3, sus7)) %>%
  # P829 add count of 1 to sus7
  mutate(sus7 = ifelse(id == "P829", sus7 + 1, sus7)) %>%
  #P890 add count of 1 to sus1
  mutate(sus1 = ifelse(id == "P890", sus1 + 1, sus1))
...
pcl <- pcl %>%
 ...
 rename_with(
  ...
  # PCL1
  mutate(pcl1_witnessed_relation_0_5 = combine_columns(., c("pcl1c1_1", "pcl1c1_2", "pcl1c1_3", "pcl1c1_9999", "pcl1c1_77"), sep = ";") )%>%
  mutate(pcl1_witnessed_relation_6_11 = combine_columns(., c("pcl1c2_1", "pcl1c2_2", "pcl1c2_3", "pcl1c2_9999", "pcl1c2_77"), sep = ";") )%>%
  mutate(pcl1_witnessed_relation_12_19 = combine_columns(., c("pcl1c3_1", "pcl1c3_2", "pcl1c3_3", "pcl1c3_9999", "pcl1c3_77"), sep = ";") )%>%
  mutate(pcl1_witnessed_relation_adult = combine_columns(., c("pcl1c4_1", "pcl1c4_2", "pcl1c4_3", "pcl1c4_9999", "pcl1c4_77"), sep = ";") )%>%
  mutate(pcl1_learned_relation = combine_columns(., c("pcl1d_1", "pcl1d_2", "pcl1d_3", "pcl1d_77"), sep = ";") )%>%
  
  relocate(pcl1_witnessed_relation_0_5, .after = "pcl1_freqadult_3_text") %>%
  relocate(pcl1_witnessed_relation_6_11, .after = "pcl1_witnessed_relation_0_5") %>%
  relocate(pcl1_witnessed_relation_12_19, .after = "pcl1_witnessed_relation_6_11") %>%
  relocate(pcl1_witnessed_relation_adult, .after = "pcl1_witnessed_relation_12_19") %>%
  relocate(pcl1_learned_relation, .after = "pcl1_witnessed_relation_adult") %>%
  ...
  # PCL2
  mutate(pcl2_witnessed_relation_0_5 = combine_columns(., c("pcl2c1_1", "pcl2c1_2", "pcl2c1_3", "pcl2c1_9999", "pcl2c1_77"), sep = ";") )%>%
  mutate(pcl2_witnessed_relation_6_11 = combine_columns(., c("pcl2c2_1", "pcl2c2_2", "pcl2c2_3", "pcl2c2_9999", "pcl2c2_77"), sep = ";") )%>%
  mutate(pcl2_witnessed_relation_12_19 = combine_columns(., c("pcl2c3_1", "pcl2c3_2", "pcl2c3_3", "pcl2c3_9999", "pcl2c3_77"), sep = ";") )%>%
  mutate(pcl2_witnessed_relation_adult = combine_columns(., c("pcl2c4_1", "pcl2c4_2", "pcl2c4_3", "pcl2c4_9999", "pcl2c4_77"), sep = ";") )%>%
  mutate(pcl2_learned_relation = combine_columns(., c("pcl2d_1", "pcl2d_2", "pcl2d_3", "pcl2d_77"), sep = ";") )%>%
  
  relocate(pcl2_witnessed_relation_0_5, .after = "pcl2_freqadult_3_text") %>%
  relocate(pcl2_witnessed_relation_6_11, .after = "pcl2_witnessed_relation_0_5") %>%
  relocate(pcl2_witnessed_relation_12_19, .after = "pcl2_witnessed_relation_6_11") %>%
  relocate(pcl2_witnessed_relation_adult, .after = "pcl2_witnessed_relation_12_19") %>%
  relocate(pcl2_learned_relation, .after = "pcl2_witnessed_relation_adult") %>%
  ...
  # PCL3
  mutate(pcl3_witnessed_relation_0_5 = combine_columns(., c("pcl3c1_1", "pcl3c1_2", "pcl3c1_3", "pcl3c1_9999", "pcl3c1_77"), sep = ";") )%>%
  mutate(pcl3_witnessed_relation_6_11 = combine_columns(., c("pcl3c2_1", "pcl3c2_2", "pcl3c2_3", "pcl3c2_9999", "pcl3c2_77"), sep = ";") )%>%
  mutate(pcl3_witnessed_relation_12_19 = combine_columns(., c("pcl3c3_1", "pcl3c3_2", "pcl3c3_3", "pcl3c3_9999", "pcl3c3_77"), sep = ";") )%>%
  mutate(pcl3_witnessed_relation_adult = combine_columns(., c("pcl3c4_1", "pcl3c4_2", "pcl3c4_3", "pcl3c4_9999", "pcl3c4_77"), sep = ";") )%>%
  mutate(pcl3_learned_relation = combine_columns(., c("pcl3d_1", "pcl3d_2", "pcl3d_3", "pcl3d_77"), sep = ";") )%>%
  
  relocate(pcl3_witnessed_relation_0_5, .after = "pcl3_freqadult_3_text") %>%
  relocate(pcl3_witnessed_relation_6_11, .after = "pcl3_witnessed_relation_0_5") %>%
  relocate(pcl3_witnessed_relation_12_19, .after = "pcl3_witnessed_relation_6_11") %>%
  relocate(pcl3_witnessed_relation_adult, .after = "pcl3_witnessed_relation_12_19") %>%    
  relocate(pcl3_learned_relation, .after = "pcl3_witnessed_relation_adult") %>%
  ...
  # PCL4
  mutate(pcl4_witnessed_relation_0_5 = combine_columns(., c("pcl4c1_1", "pcl4c1_2", "pcl4c1_3", "pcl4c1_9999", "pcl4c1_77"), sep = ";") )%>%
  mutate(pcl4_witnessed_relation_6_11 = combine_columns(., c("pcl4c2_1", "pcl4c2_2", "pcl4c2_3", "pcl4c2_9999", "pcl4c2_77"), sep = ";") )%>%
  mutate(pcl4_witnessed_relation_12_19 = combine_columns(., c("pcl4c3_1", "pcl4c3_2", "pcl4c3_3", "pcl4c3_9999", "pcl4c3_77"), sep = ";") )%>%
  mutate(pcl4_witnessed_relation_adult = combine_columns(., c("pcl4c4_1", "pcl4c4_2", "pcl4c4_3", "pcl4c4_9999", "pcl4c4_77"), sep = ";") )%>%
  mutate(pcl4_learned_relation = combine_columns(., c("pcl4d_1", "pcl4d_2", "pcl4d_3", "pcl4d_77"), sep = ";") )%>%
 
  relocate(pcl4_witnessed_relation_0_5, .after = "pcl4_freqadult_3_text") %>%
  relocate(pcl4_witnessed_relation_6_11, .after = "pcl4_witnessed_relation_0_5") %>%
  relocate(pcl4_witnessed_relation_12_19, .after = "pcl4_witnessed_relation_6_11") %>%
  relocate(pcl4_witnessed_relation_adult, .after = "pcl4_witnessed_relation_12_19") %>%
  relocate(pcl4_learned_relation, .after = "pcl4_witnessed_relation_adult") %>%
  ...
  # PCL5
  mutate(pcl5_witnessed_relation_0_5 = combine_columns(., c("pcl5c1_1", "pcl5c1_2", "pcl5c1_3", "pcl5c1_9999", "pcl5c1_77"), sep = ";") )%>%
  mutate(pcl5_witnessed_relation_6_11 = combine_columns(., c("pcl5c2_1", "pcl5c2_2", "pcl5c2_3", "pcl5c2_9999", "pcl5c2_77"), sep = ";") )%>%
  mutate(pcl5_witnessed_relation_12_19 = combine_columns(., c("pcl5c3_1", "pcl5c3_2", "pcl5c3_3", "pcl5c3_9999", "pcl5c3_77"), sep = ";") )%>%
  mutate(pcl5_witnessed_relation_adult = combine_columns(., c("pcl5c4_1", "pcl5c4_2", "pcl5c4_3", "pcl5c4_9999", "pcl5c4_77"), sep = ";") )%>%
  mutate(pcl5_learned_relation = combine_columns(., c("pcl5d_1", "pcl5d_2", "pcl5d_3", "pcl5d_77"), sep = ";") )%>%
  
  relocate(pcl5_witnessed_relation_0_5, .after = "pcl5_freqadult_3_text") %>%
  relocate(pcl5_witnessed_relation_6_11, .after = "pcl5_witnessed_relation_0_5") %>%
  relocate(pcl5_witnessed_relation_12_19, .after = "pcl5_witnessed_relation_6_11") %>%
  relocate(pcl5_witnessed_relation_adult, .after = "pcl5_witnessed_relation_12_19") %>%
  relocate(pcl5_learned_relation, .after = "pcl5_witnessed_relation_adult") %>%
  select(-c("pcl5c1_1", "pcl5c1_2", "pcl5c1_3", "pcl5c1_9999", "pcl5c1_77",
    ...
    # PCL6
  mutate(pcl6_witnessed_relation_0_5 = combine_columns(., c("pcl6c1_1", "pcl6c1_2", "pcl6c1_3", "pcl6c1_9999", "pcl6c1_77"), sep = ";") )%>%
  mutate(pcl6_witnessed_relation_6_11 = combine_columns(., c("pcl6c2_1", "pcl6c2_2", "pcl6c2_3", "pcl6c2_9999", "pcl6c2_77"), sep = ";") )%>%
  mutate(pcl6_witnessed_relation_12_19 = combine_columns(., c("pcl6c3_1", "pcl6c3_2", "pcl6c3_3", "pcl6c3_9999", "pcl6c3_77"), sep = ";") )%>%
  mutate(pcl6_witnessed_relation_adult = combine_columns(., c("pcl6c4_1", "pcl6c4_2", "pcl6c4_3", "pcl6c4_9999", "pcl6c4_77"), sep = ";") )%>%
  mutate(pcl6_learned_relation = combine_columns(., c("pcl6d_1", "pcl6d_2", "pcl6d_3", "pcl6d_77"), sep = ";") )%>%
  
  relocate(pcl6_witnessed_relation_0_5, .after = "pcl6_freqadult_3_text") %>%
  relocate(pcl6_witnessed_relation_6_11, .after = "pcl6_witnessed_relation_0_5") %>%
  relocate(pcl6_witnessed_relation_12_19, .after = "pcl6_witnessed_relation_6_11") %>%
  relocate(pcl6_witnessed_relation_adult, .after = "pcl6_witnessed_relation_12_19") %>%
  relocate(pcl6_learned_relation, .after = "pcl6_witnessed_relation_adult") %>%
  select(-c("pcl6c1_1", "pcl6c1_2", "pcl6c1_3", "pcl6c1_9999", "pcl6c1_77",
    ...
    # PCL7
  mutate(pcl7_witnessed_relation_0_5 = combine_columns(., c("pcl7c1_1", "pcl7c1_2", "pcl7c1_3", "pcl7c1_9999", "pcl7c1_77"), sep = ";") )%>%
  mutate(pcl7_witnessed_relation_6_11 = combine_columns(., c("pcl7c2_1", "pcl7c2_2", "pcl7c2_3", "pcl7c2_9999", "pcl7c2_77"), sep = ";") )%>%
  mutate(pcl7_witnessed_relation_12_19 = combine_columns(., c("pcl7c3_1", "pcl7c3_2", "pcl7c3_3", "pcl7c3_9999", "pcl7c3_77"), sep = ";") )%>%
  mutate(pcl7_witnessed_relation_adult = combine_columns(., c("pcl7c4_1", "pcl7c4_2", "pcl7c4_3", "pcl7c4_9999", "pcl7c4_77"), sep = ";") )%>%
  mutate(pcl7_learned_relation = combine_columns(., c("pcl7d_1", "pcl7d_2", "pcl7d_3", "pcl7d_77"), sep = ";") )%>%
  relocate(pcl7_witnessed_relation_0_5, .after = "pcl7_freqadult_3_text") %>%
  relocate(pcl7_witnessed_relation_6_11, .after = "pcl7_witnessed_relation_0_5") %>%
  relocate(pcl7_witnessed_relation_12_19, .after = "pcl7_witnessed_relation_6_11") %>%
  relocate(pcl7_witnessed_relation_adult, .after = "pcl7_witnessed_relation_12_19") %>%
  relocate(pcl7_learned_relation, .after = "pcl7_witnessed_relation_adult") %>%
  select(-c("pcl7c1_1", "pcl7c1_2", "pcl7c1_3", "pcl7c1_9999", "pcl7c1_77",
    ...
    # PCL8
  mutate(pcl8_witnessed_relation_0_5 = combine_columns(., c("pcl8c1_1", "pcl8c1_2", "pcl8c1_3", "pcl8c1_9999", "pcl8c1_77"), sep = ";") )%>%
  mutate(pcl8_witnessed_relation_6_11 = combine_columns(., c("pcl8c2_1", "pcl8c2_2", "pcl8c2_3", "pcl8c2_9999", "pcl8c2_77"), sep = ";") )%>%
  mutate(pcl8_witnessed_relation_12_19 = combine_columns(., c("pcl8c3_1", "pcl8c3_2", "pcl8c3_3", "pcl8c3_9999", "pcl8c3_77"), sep = ";") )%>%
  mutate(pcl8_witnessed_relation_adult = combine_columns(., c("pcl8c4_1", "pcl8c4_2", "pcl8c4_3", "pcl8c4_9999", "pcl8c4_77"), sep = ";") )%>%
  mutate(pcl8_learned_relation = combine_columns(., c("pcl8d_1", "pcl8d_2", "pcl8d_3", "pcl8d_77"), sep = ";") )%>%
  relocate(pcl8_witnessed_relation_0_5, .after = "pcl8_freqadult_3_text") %>%
  relocate(pcl8_witnessed_relation_6_11, .after = "pcl8_witnessed_relation_0_5") %>%
  relocate(pcl8_witnessed_relation_12_19, .after = "pcl8_witnessed_relation_6_11") %>%
  relocate(pcl8_witnessed_relation_adult, .after = "pcl8_witnessed_relation_12_19") %>%
  relocate(pcl8_learned_relation, .after = "pcl8_witnessed_relation_adult") %>%
  select(-c("pcl8c1_1", "pcl8c1_2", "pcl8c1_3", "pcl8c1_9999", "pcl8c1_77",
    ...
    # PCL9
  mutate(pcl9_witnessed_relation_0_5 = combine_columns(., c("pcl9c1_1", "pcl9c1_2", "pcl9c1_3", "pcl9c1_9999", "pcl9c1_77"), sep = ";") )%>%
  mutate(pcl9_witnessed_relation_6_11 = combine_columns(., c("pcl9c2_1", "pcl9c2_2", "pcl9c2_3", "pcl9c2_9999", "pcl9c2_77"), sep = ";") )%>%
  mutate(pcl9_witnessed_relation_12_19 = combine_columns(., c("pcl9c3_1", "pcl9c3_2", "pcl9c3_3", "pcl9c3_9999", "pcl9c3_77"), sep = ";") )%>%
  mutate(pcl9_witnessed_relation_adult = combine_columns(., c("pcl9c4_1", "pcl9c4_2", "pcl9c4_3", "pcl9c4_9999", "pcl9c4_77"), sep = ";") )%>%
  mutate(pcl9_learnsed_relation = combine_columns(., c("pcl9d_1", "pcl9d_2", "pcl9d_3", "pcl9d_77"), sep = ";") )%>%
  relocate(pcl9_witnessed_relation_0_5, .after = "pcl9_freqadult_3_text") %>%
  relocate(pcl9_witnessed_relation_6_11, .after = "pcl9_witnessed_relation_0_5") %>%
  relocate(pcl9_witnessed_relation_12_19, .after = "pcl9_witnessed_relation_6_11") %>%
  relocate(pcl9_witnessed_relation_adult, .after = "pcl9_witnessed_relation_12_19") %>%
  relocate(pcl9_learnsed_relation, .after = "pcl9_witnessed_relation_adult") %>%
  select(-c("pcl9c1_1", "pcl9c1_2", "pcl9c1_3", "pcl9c1_9999", "pcl9c1_77",
    ...
    # PCL10
  mutate(pcl10_witnessed_relation_0_5 = combine_columns(., c("pcl10c1_1", "pcl10c1_2", "pcl10c1_3", "pcl10c1_9999", "pcl10c1_77"), sep = ";") )%>%
  mutate(pcl10_witnessed_relation_6_11 = combine_columns(., c("pcl10c2_1", "pcl10c2_2", "pcl10c2_3", "pcl10c2_9999", "pcl10c2_77"), sep = ";") )%>%
  mutate(pcl10_witnessed_relation_12_19 = combine_columns(., c("pcl10c3_1", "pcl10c3_2", "pcl10c3_3", "pcl10c3_9999", "pcl10c3_77"), sep = ";") )%>%
  mutate(pcl10_witnessed_relation_adult = combine_columns(., c("pcl10c4_1", "pcl10c4_2", "pcl10c4_3", "pcl10c4_9999",  "pcl10c4_77"), sep = ";") )%>%
  mutate(pcl10_learned_relation = combine_columns(., c("pcl10d_1", "pcl10d_2", "pcl10d_3", "pcl10d_77"), sep = ";") )%>%
  relocate(pcl10_witnessed_relation_0_5, .after = "pcl10_freqadult_3_text") %>%
  relocate(pcl10_witnessed_relation_6_11, .after = "pcl10_witnessed_relation_0_5") %>%
  relocate(pcl10_witnessed_relation_12_19, .after = "pcl10_witnessed_relation_6_11") %>%
  relocate(pcl10_witnessed_relation_adult, .after = "pcl10_witnessed_relation_12_19") %>%
  relocate(pcl10_learned_relation, .after = "pcl10_witnessed_relation_adult") %>%
  select(-c("pcl10c1_1", "pcl10c1_2", "pcl10c1_3", "pcl10c1_9999", "pcl10c1_77",
    ...
    # PCL11
  mutate(pcl11_witnessed_relation_0_5 = combine_columns(., c("pcl11c_1", "pcl11c_2", "pcl11c_3", "pcl11c_9999", "pcl11c_77"), sep = ";") )%>%
  mutate(pcl11_witnessed_relation_6_11 = combine_columns(., c("pcl11c2_1", "pcl11c2_2", "pcl11c2_3", "pcl11c2_9999", "pcl11c2_77"), sep = ";") )%>%
  mutate(pcl11_witnessed_relation_12_19 = combine_columns(., c("pcl11c3_1", "pcl11c3_2", "pcl11c3_3", "pcl11c3_9999",  "pcl11c3_77"), sep = ";") )%>%
  mutate(pcl11_witnessed_relation_adult = combine_columns(., c("pcl11c4_1", "pcl11c4_2", "pcl11c4_3", "pcl11c4_9999", "pcl11c4_77"), sep = ";") )%>%
  mutate(pcl11_learned_relation = combine_columns(., c("pcl11d_1", "pcl11d_2", "pcl11d_3", "pcl11d_77"), sep = ";") )%>%
  relocate(pcl11_witnessed_relation_0_5, .after = "pcl11_freqadult_3_text") %>%
  relocate(pcl11_witnessed_relation_6_11, .after = "pcl11_witnessed_relation_0_5") %>%
  relocate(pcl11_witnessed_relation_12_19, .after = "pcl11_witnessed_relation_6_11") %>%
  relocate(pcl11_witnessed_relation_adult, .after = "pcl11_witnessed_relation_12_19") %>%
  relocate(pcl11_learned_relation, .after = "pcl11_witnessed_relation_adult") %>%
  select(-c("pcl11c_1", "pcl11c_2", "pcl11c_3", "pcl11c_9999", "pcl11c_77",
    ...
    # PCL12
  mutate(pcl12_witnessed_relation_0_5 = combine_columns(., c("pcl12c1_1", "pcl12c1_2", "pcl12c1_3", "pcl12c1_9999", "pcl12c1_77"), sep = ";") )%>%
  mutate(pcl12_witnessed_relation_6_11 = combine_columns(., c("pcl12c2_1", "pcl12c2_2", "pcl12c2_3", "pcl12c2_9999", "pcl12c2_77"), sep = ";") )%>%
  mutate(pcl12_witnessed_relation_12_19 = combine_columns(., c("pcl12c3_1", "pcl12c3_2", "pcl12c3_3", "pcl12c3_9999", "pcl12c3_77"), sep = ";") )%>%
  mutate(pcl12_witnessed_relation_adult = combine_columns(., c("pcl12c4_1", "pcl12c4_2", "pcl12c4_3", "pcl12c4_9999", "pcl12c4_77"), sep = ";") )%>%
  mutate(pcl12_learned_relation = combine_columns(., c("pcl12d_1", "pcl12d_2", "pcl12d_3", "pcl12d_77"), sep = ";") )%>%
  relocate(pcl12_witnessed_relation_0_5, .after = "pcl12_freqadult_3_text") %>%
  relocate(pcl12_witnessed_relation_6_11, .after = "pcl12_witnessed_relation_0_5") %>%
  relocate(pcl12_witnessed_relation_12_19, .after = "pcl12_witnessed_relation_6_11") %>%
  relocate(pcl12_witnessed_relation_adult, .after = "pcl12_witnessed_relation_12_19") %>%
  relocate(pcl12_learned_relation, .after = "pcl12_witnessed_relation_adult") %>%
  select(-c("pcl12c1_1", "pcl12c1_2", "pcl12c1_3", "pcl12c1_9999", "pcl12c1_77",
      ...
      # PCL13
  mutate(pcl13_witnessed_relation_0_5 = combine_columns(., c("pcl13c1_1", "pcl13c1_2", "pcl13c1_3", "pcl13c1_9999", "pcl13c1_77"), sep = ";") )%>%
  mutate(pcl13_witnessed_relation_6_11 = combine_columns(., c("pcl13c2_1", "pcl13c2_2", "pcl13c2_3", "pcl13c2_9999", "pcl13c2_77"), sep = ";") )%>%
  mutate(pcl13_witnessed_relation_12_19 = combine_columns(., c("pcl13c3_1", "pcl13c3_2", "pcl13c3_3", "pcl13c3_9999", "pcl13c3_77"), sep = ";") )%>%
  mutate(pcl13_witnessed_relation_adult = combine_columns(., c("pcl13c4_1", "pcl13c4_2", "pcl13c4_3", "pcl13c4_9999",  "pcl13c4_77"), sep = ";") )%>%
  mutate(pcl13_learned_relation = combine_columns(., c("pcl13d_1", "pcl13d_2", "pcl13d_3", "pcl13d_77"), sep = ";") )%>%
  relocate(pcl13_witnessed_relation_0_5, .after = "pcl13_freqadult_3_text") %>%
  relocate(pcl13_witnessed_relation_6_11, .after = "pcl13_witnessed_relation_0_5") %>%
  relocate(pcl13_witnessed_relation_12_19, .after = "pcl13_witnessed_relation_6_11") %>%
  relocate(pcl13_witnessed_relation_adult, .after = "pcl13_witnessed_relation_12_19") %>%
  relocate(pcl13_learned_relation, .after = "pcl13_witnessed_relation_adult") %>%
  select(-c("pcl13c1_1", "pcl13c1_2", "pcl13c1_3", "pcl13c1_9999", "pcl13c1_77",
      ...
      # PCL14
  mutate(pcl14_witnessed_relation_0_5 = combine_columns(., c("pcl14c1_1", "pcl14c1_2", "pcl14c1_3", "pcl14c1_9999", "pcl14c1_77"), sep = ";") )%>%
  mutate(pcl14_witnessed_relation_6_11 = combine_columns(., c("pcl14c2_1", "pcl14c2_2", "pcl14c2_3", "pcl14c2_9999", "pcl14c2_77"), sep = ";") )%>%
  mutate(pcl14_witnessed_relation_12_19 = combine_columns(., c("pcl14c3_1", "pcl14c3_2", "pcl14c3_3", "pcl14c3_9999", "pcl14c3_77"), sep = ";") )%>%
  mutate(pcl14_witnessed_relation_adult = combine_columns(., c("pcl14c4_1", "pcl14c4_2", "pcl14c4_3", "pcl14c4_9999", "pcl14c4_77"), sep = ";") )%>%
  mutate(pcl14_learned_relation = combine_columns(., c("pcl14d_1", "pcl14d_2", "pcl14d_3", "pcl14d_77"), sep = ";") )%>%
  relocate(pcl14_witnessed_relation_0_5, .after = "pcl14_freqadult_3_text") %>%
  relocate(pcl14_witnessed_relation_6_11, .after = "pcl14_witnessed_relation_0_5") %>%
  relocate(pcl14_witnessed_relation_12_19, .after = "pcl14_witnessed_relation_6_11") %>%
  relocate(pcl14_witnessed_relation_adult, .after = "pcl14_witnessed_relation_12_19") %>%
  relocate(pcl14_learned_relation, .after = "pcl14_witnessed_relation_adult") %>%
  select(-c("pcl14c1_1", "pcl14c1_2", "pcl14c1_3", "pcl14c1_9999", "pcl14c1_77",
      ...
      # PCL15
  mutate(pcl15_witnessed_relation_0_5 = combine_columns(., c("pcl15c1_1", "pcl15c1_2", "pcl15c1_3", "pcl15c1_9999", "pcl15c1_77"), sep = ";") )%>%
  mutate(pcl15_witnessed_relation_6_11 = combine_columns(., c("pcl15c2_1", "pcl15c2_2", "pcl15c2_3", "pcl15c2_9999",  "pcl15c2_77"), sep = ";") )%>%
  mutate(pcl15_witnessed_relation_12_19 = combine_columns(., c("pcl15c3_1", "pcl15c3_2", "pcl15c3_3", "pcl15c3_9999", "pcl15c3_77"), sep = ";") )%>%
  mutate(pcl15_witnessed_relation_adult = combine_columns(., c("pcl15c4_1", "pcl15c4_2", "pcl15c4_3", "pcl15c4_9999", "pcl15c4_77"), sep = ";") )%>%
  mutate(pcl15_learned_relation = combine_columns(., c("pcl15d_1", "pcl15d_2", "pcl15d_3", "pcl15d_77"), sep = ";") )%>%
  relocate(pcl15_witnessed_relation_0_5, .after = "pcl15_freqadult_3_text") %>%
  relocate(pcl15_witnessed_relation_6_11, .after = "pcl15_witnessed_relation_0_5") %>%
  relocate(pcl15_witnessed_relation_12_19, .after = "pcl15_witnessed_relation_6_11") %>%
  relocate(pcl15_witnessed_relation_adult, .after = "pcl15_witnessed_relation_12_19") %>%
  relocate(pcl15_learned_relation, .after = "pcl15_witnessed_relation_adult") %>%
  select(-c("pcl15c1_1", "pcl15c1_2", "pcl15c1_3", "pcl15c1_9999", "pcl15c1_77",
    ...
    # PCL16
  mutate(pcl16_witnessed_relation_0_5 = combine_columns(., c("pcl16c1_1", "pcl16c1_2", "pcl16c1_3", "pcl16c1_9999", "pcl16c1_77"), sep = ";") )%>%
  mutate(pcl16_witnessed_relation_6_11 = combine_columns(., c("pcl16c2_1", "pcl16c2_2", "pcl16c2_3", "pcl16c2_9999",  "pcl16c2_77"), sep = ";") )%>%
  mutate(pcl16_witnessed_relation_12_19 = combine_columns(., c("pcl16c3_1", "pcl16c3_2", "pcl16c3_3", "pcl16c3_9999", "pcl16c3_77"), sep = ";") )%>%
  mutate(pcl16_witnessed_relation_adult = combine_columns(., c("pcl16c4_1", "pcl16c4_2", "pcl16c4_3",  "pcl16c4_9999", "pcl16c4_77"), sep = ";") )%>%
  mutate(pcl16_learned_relation = combine_columns(., c("pcl16d_1", "pcl16d_2", "pcl16d_3", "pcl16d_77"), sep = ";") )%>%
  relocate(pcl16_witnessed_relation_0_5, .after = "pcl16_freqadult_3_text") %>%
  relocate(pcl16_witnessed_relation_6_11, .after = "pcl16_witnessed_relation_0_5") %>%
  relocate(pcl16_witnessed_relation_12_19, .after = "pcl16_witnessed_relation_6_11") %>%
  relocate(pcl16_witnessed_relation_adult, .after = "pcl16_witnessed_relation_12_19") %>%
  relocate(pcl16_learned_relation, .after = "pcl16_witnessed_relation_adult") %>%
  select(-c("pcl16c1_1", "pcl16c1_2", "pcl16c1_3", "pcl16c1_9999", "pcl16c1_77",
      ...
      # PCL17
  mutate(pcl17_witnessed_relation_0_5 = combine_columns(., c("pcl17c1_1", "pcl17c1_2", "pcl17c1_3", "pcl17c1_9999", "pcl17c1_77"), sep = ";") )%>%
  mutate(pcl17_witnessed_relation_6_11 = combine_columns(., c("pcl17c2_1", "pcl17c2_2", "pcl17c2_3", "pcl17c2_9999", "pcl17c2_77"), sep = ";") )%>%
  mutate(pcl17_witnessed_relation_12_19 = combine_columns(., c("pcl17c3_1", "pcl17c3_2", "pcl17c3_3", "pcl17c3_9999", "pcl17c3_77"), sep = ";") )%>%
  mutate(pcl17_witnessed_relation_adult = combine_columns(., c("pcl17c4_1", "pcl17c4_2", "pcl17c4_3", "pcl17c4_9999", "pcl17c4_77"), sep = ";") )%>%
  mutate(pcl17_learned_relation = combine_columns(., c("pcl17d_1", "pcl17d_2", "pcl17d_3", "pcl17d_77"), sep = ";") )%>%
  relocate(pcl17_witnessed_relation_0_5, .after = "pcl17_freqadult_3_text") %>%
  relocate(pcl17_witnessed_relation_6_11, .after = "pcl17_witnessed_relation_0_5") %>%
  relocate(pcl17_witnessed_relation_12_19, .after = "pcl17_witnessed_relation_6_11") %>%
  relocate(pcl17_witnessed_relation_adult, .after = "pcl17_witnessed_relation_12_19") %>%
  relocate(pcl17_learned_relation, .after = "pcl17_witnessed_relation_adult") %>%
  select(-c("pcl17c1_1", "pcl17c1_2", "pcl17c1_3", "pcl17c1_9999", "pcl17c1_77",
      ...
      # PCL18
  mutate(pcl18_witnessed_relation_0_5 = combine_columns(., c("pcl18c1_1", "pcl18c1_2", "pcl18c1_3", "pcl18c1_9999", "pcl18c1_77"), sep = ";") )%>%
  mutate(pcl18_witnessed_relation_6_11 = combine_columns(., c("pcl18c2_1", "pcl18c2_2", "pcl18c2_3", "pcl18c2_9999", "pcl18c2_77"), sep = ";") )%>%
  mutate(pcl18_witnessed_relation_12_19 = combine_columns(., c("pcl18c3_1", "pcl18c3_2", "pcl18c3_3", "pcl18c3_9999", "pcl18c3_77"), sep = ";") )%>%
  mutate(pcl18_witnessed_relation_adult = combine_columns(., c("pcl18c4_1", "pcl18c4_2", "pcl18c4_3", "pcl18c4_9999", "pcl18c4_77"), sep = ";") )%>%
  mutate(pcl18_learned_relation = combine_columns(., c("pcl18d_1", "pcl18d_2", "pcl18d_3", "pcl18d_77"), sep = ";") )%>%
  relocate(pcl18_witnessed_relation_0_5, .after = "pcl18_freqadult_3_text") %>%
  relocate(pcl18_witnessed_relation_6_11, .after = "pcl18_witnessed_relation_0_5") %>%
  relocate(pcl18_witnessed_relation_12_19, .after = "pcl18_witnessed_relation_6_11") %>%
  relocate(pcl18_witnessed_relation_adult, .after = "pcl18_witnessed_relation_12_19") %>%
  relocate(pcl18_learned_relation, .after = "pcl18_witnessed_relation_adult") %>%
  select(-c("pcl18c1_1", "pcl18c1_2", "pcl18c1_3", "pcl18c1_9999", "pcl18c1_77",
      ...
      # PCL19
  mutate(pcl19_witnessed_relation_0_5 = combine_columns(., c("pcl19c1_1", "pcl19c1_2", "pcl19c1_3", "pcl19c1_9999", "pcl19c1_77"), sep = ";") )%>%
  mutate(pcl19_witnessed_relation_6_11 = combine_columns(., c("pcl19c2_1", "pcl19c2_2", "pcl19c2_3", "pcl19c2_9999", "pcl19c2_77"), sep = ";") )%>%
  mutate(pcl19_witnessed_relation_12_19 = combine_columns(., c("pcl19c3_1", "pcl19c3_2", "pcl19c3_3", "pcl19c3_9999", "pcl19c3_77"), sep = ";") )%>%
  mutate(pcl19_witnessed_relation_adult = combine_columns(., c("pcl19c4_1", "pcl19c4_2", "pcl19c4_3", "pcl19c4_9999", "pcl19c4_77"), sep = ";") )%>%
  mutate(pcl19_learned_relation = combine_columns(., c("pcl19d_1", "pcl19d_2", "pcl19d_3", "pcl19d_77"), sep = ";") )%>%
  relocate(pcl19_witnessed_relation_0_5, .after = "pcl19_freqadult_3_text") %>%
  relocate(pcl19_witnessed_relation_6_11, .after = "pcl19_witnessed_relation_0_5") %>%
  relocate(pcl19_witnessed_relation_12_19, .after = "pcl19_witnessed_relation_6_11") %>%
  relocate(pcl19_witnessed_relation_adult, .after = "pcl19_witnessed_relation_12_19") %>%
  relocate(pcl19_learned_relation, .after = "pcl19_witnessed_relation_adult") %>%
...
for (column in columns_happen) {
  pcl <- pcl %>%
    mutate({{column}} := case_when(
      #When _77 is -77, then change _happen to -77
      get(paste0("pcl",gsub("[^0-9]", "", column), "_77")) == "-77" ~ "-77",
      # if _happen is na, and _77 is NA, then _happen is 0 
...
for (column in columns_witness) {
  pcl <- pcl %>%
    mutate({{column}} := case_when(
      #When _77 is -77, then change _witness to -77
      get(paste0("pcl",gsub("[^0-9]", "", column), "_77")) == "-77" ~ "-77",
      # if _witness is na, and _77 is NA, then _witness is 0 
      is.na(get(column)) & is.na(get(paste0("pcl",gsub("[^0-9]", "", column), "_77"))) ~ "0",
      TRUE ~ get(column)
    ))
}
...
for (column in columns_learn) {
  pcl <- pcl %>%
    mutate({{column}} := case_when(
      #When _77 is -77, then change _learn to -77
      get(paste0("pcl",gsub("[^0-9]", "", column), "_77")) == "-77" ~ "-77",
       # if _learn is na, and _77 is NA, then _learn is 0 
      is.na(get(column)) & is.na(get(paste0("pcl",gsub("[^0-9]", "", column), "_77"))) ~ "0",
      TRUE ~ get(column)
    ))
}
...
for (column in columns_no) {
  pcl <- pcl %>%
    mutate({{column}} := case_when(
      #When _77 is -77, then change _no to -77
      get(paste0("pcl",gsub("[^0-9]", "", column), "_77")) == "-77" ~ "-77",
       # if _no is na, and _77 is NA, then _no is 0 
      is.na(get(column)) & is.na(get(paste0("pcl",gsub("[^0-9]", "", column), "_77"))) ~ "0",
      TRUE ~ get(column)
    ))
}
...
for (column in columns_hapWit_ages_0_5) {
  pcl <- pcl %>%
    mutate({{column}} := case_when(
      #When _hapWit_ages_77 is -77, then change _hapWit_ages_0_5 to -77
      get(paste0("pcl", gsub("pcl(\\d+).*", "\\1", column), "_hapWit_ages_77")) == "-77" ~ "-77",
      # if _hapWit_ages_0_5 is na, and _hapWit_ages_77 is NA, then _hapWit_ages_77 is 0 
...
for (column in columns_hapWit_ages_6_11) {
  pcl <- pcl %>%
    mutate({{column}} := case_when(
      #When _hapWit_ages_77 is -77, then change _hapWit_ages_6_11 to -77
      get(paste0("pcl", gsub("pcl(\\d+).*", "\\1", column), "_hapWit_ages_77")) == "-77" ~ "-77",
      # if _hapWit_ages_6_11 is na, and _hapWit_ages_77 is NA, then _hapWit_ages_77 is 0 
      is.na(get(column)) & is.na(get(paste0("pcl", gsub("pcl(\\d+).*", "\\1", column), "_hapWit_ages_77"))) ~ "0",
      TRUE ~ get(column)
    ))
}
...
for (column in columns_hapWit_ages_12_19) {
  pcl <- pcl %>%
    mutate({{column}} := case_when(
      #When _hapWit_ages_77 is -77, then change _hapWit_ages_12_19 to -77
      get(paste0("pcl", gsub("pcl(\\d+).*", "\\1", column), "_hapWit_ages_77")) == "-77" ~ "-77",
      # if _hapWit_ages_12_19 is na, and _hapWit_ages_77 is NA, then _hapWit_ages_77 is 0 
      is.na(get(column)) & is.na(get(paste0("pcl", gsub("pcl(\\d+).*", "\\1", column), "_hapWit_ages_77"))) ~ "0",
      TRUE ~ get(column)
    ))
}
...
for (column in columns_hapWit_adulthood) {
  pcl <- pcl %>%
    mutate({{column}} := case_when(
       #When _hapWit_ages_77 is -77, then change _hapWit_adulthood to -77
      get(paste0("pcl", gsub("pcl(\\d+).*", "\\1", column), "_hapWit_ages_77")) == "-77" ~ "-77",
       # if _hapWit_adulthood is na, and _hapWit_ages_77 is NA, then _hapWit_ages_77 is 0 
      is.na(get(column)) & is.na(get(paste0("pcl", gsub("pcl(\\d+).*", "\\1", column), "_hapWit_ages_77"))) ~ "0",
      TRUE ~ get(column)
    ))
}
...
for (column in columns_hapWit_dev) {
  pcl <- pcl %>%
    mutate({{column}} := case_when(
      # if _hapWit_ages_77 is -77 then _hapWit_dev is -77
      get(paste0("pcl", gsub("pcl(\\d+).*", "\\1", column), "_hapWit_ages_77")) == "-77" ~ "-77",
      # if it's still NA then change to 0 
      is.na(get(column)) ~ "0",
      TRUE ~ get(column)
    ))
}
...
# Frequency ##################################################################################
# correct frequency 
#if pcl1_hapWit_ages_1_5 - NA then move pcl1_freq0_5 to pcl1b? 
# # if pcl1_hapWit_ages_1_5 is.na, then pcl1_freq0-5 should be na
...
for (column in columns_freq0_5_text) {
    pcl <- pcl %>%
      mutate({{column}} := case_when(    
        # if _freq0-5 is not 2 then _freq0-5_text should be -55
...
for (column in columns_freq6_11_text) {
    pcl <- pcl %>%
      mutate({{column}} := case_when(    
        # if _freq6-11 is not 2 then _freq6-11_text should be -55
...
for (column in columns_freq12_19_text) {
    pcl <- pcl %>%
      mutate({{column}} := case_when(    
        # if _freq12-19 is not 2 then _freq12-19_text should be -55 
...
for (column in columns_freqadult_text) {
    pcl <- pcl %>%
      mutate({{column}} := case_when(   
        # if_freqadult is not 2 then _freqadult_text_text should be -55 
...
# relation  ##################################################################################
columns_relation <- c(paste0("pcl", 1:19, "_witnessed_relation_0_5"), paste0("pcl", 1:19, "_witnessed_relation_6_11"), paste0("pcl", 1:19, "_witnessed_relation_12_19"), paste0("pcl", 1:19, "_witnessed_relation_adult"))
for (column in columns_relation) {
    pcl <- pcl %>%
      mutate({{column}} := case_when(       
        # if _witness is 0 and _learn is 0 then _relation is -55
        get(paste0("pcl", gsub("pcl(\\d+).*", "\\1", column), "_witness")) == "0" ~ "-55",
        # if _witness is -77/-55/-99 and _learn is -77/-55/-99 then _relation is -55
        get(paste0("pcl", gsub("pcl(\\d+).*", "\\1", column), "_witness")) %in% c("-77","-55", "-99")  ~ "-55",
        get(column) == "" ~ "-99",
        is.na(get(column)) ~ "-99",
        TRUE ~ get(column)
      ))
...
for (column in columns_devperiod) {
    pcl <- pcl %>%
      mutate({{column}} := case_when(   
        # if _happen is 0 then _devperiod should be -55
        get(paste0("pcl", gsub("pcl(\\d+).*", "\\1", column), "_happen")) == "0" ~ "-55",
        # if _relation are still NA or empty then _devperiod should be truly missing 
        is.na(get(column)) ~ "-99",
        TRUE ~ get(column)
      ))
}
...
# if _hapWit_dev is na, then change _hapWit_dev to 0
pcl <- pcl %>%
...
pcl_complete <- pcl_calc %>%
  ...
  merge(pcl, by = "id") %>%
  # Dropping these variables for archive. 
  select(-c(pcl1b:pcl19b_text, matches("pcl\\d+_77"), matches("pcl\\d+_hapWit_ages_77"))) %>%
...
pcl_internal_old_variable <- pcl %>%
  # when using frequency please double check with original raw data 
...
tpw_archive_calc <- tpw_archive_id %>%
  left_join(parent_complete, by = "id") %>%
  left_join(cls_data, by = "id") %>%
  left_join(arril_complete, by = "id") %>%
  left_join(arrest_complete, by = "id") %>%
...
write.csv(tpw_archive_calc, here("do_not_push", "tpw_archive.csv"))
